...
func RecallGlobalHot(ctx context.Context, request Request) ([]*model.Candidate, error) {
	if request.PoolStore != nil {
		...
		if poolErr == nil {
			list, buildErr := buildPoolGoodsCandidates(ctx, request.Dependencies.Goods, pool.GetItems(), RecallSourceGlobalHot, func(candidate *model.Candidate, score float64) {
				candidate.Score.GlobalHotScore = score
			})
			if buildErr != nil {
				return nil, buildErr
			}
			// 非个性化池存在且能恢复全站热度原始分值时，优先使用离线池结果。
			if len(list) > 0 {
				return list, nil
			}
		}
