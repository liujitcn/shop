...
// McpHTTPHandler 包装 MCP 标准 HTTP 处理器，便于注入到统一 HTTP 服务。
type McpHTTPHandler struct {
	...
	toolOperationMap map[string]string
}

// NewMcpHTTPHandler 创建 MCP HTTP 处理器，并从 base_api 表加载接口工具。
func NewMcpHTTPHandler(ctx *bootstrap.Context, baseAPIRepo *data.BaseAPIRepository) (*McpHTTPHandler, error) {
	h := &McpHTTPHandler{
		baseAPIRepo:      baseAPIRepo,
		toolOperationMap: make(map[string]string),
	}
	cfg := ctx.GetConfig()
	...
	h.httpClient = h.newHTTPClient(httpTimeout)
	h.openAPISchemas = h.loadOpenAPISchemas()
	mcpOptions = append(mcpOptions, mcpGoServer.WithToolFilter(h.filterTools))
	serverOptions = append(serverOptions, rpc.WithMcpServerOptions(mcpOptions...))

	var err error
	h.server, err = rpc.CreateMcpHandler(cfg, serverOptions...)
	if err != nil {
		return nil, err
	}
	...
	if cfg.Server.Http != nil {
		h.httpEndpoint = h.normalizeHTTPEndpoint(cfg.Server.Http.GetAddr())
	}

	err = h.registerBaseAPITools()
	if err != nil {
		return nil, err
	}
	h.Handler, err = h.server.HTTPHandler()
	if err != nil {
		return nil, err
	}
	if h.Handler == nil {
...
func (h *McpHTTPHandler) registerBaseAPITools() error {
	var err error
	...
	for _, baseAPI := range apiList {
		...
		argMapping := strings.TrimSpace(baseAPI.ArgMapping)
		if argMapping != "" {
			err = json.Unmarshal([]byte(argMapping), &mappings)
			if err != nil {
				log.Errorf("解析 MCP 参数映射失败 operation=%s err=%v", baseAPI.Operation, err)
				continue
			}
		}
		toolName, description, inputSchema := h.buildToolDefinition(baseAPI, toolNames, h.openAPISchemas)
		h.toolOperationMap[toolName] = baseAPI.Operation
		currentAPI := baseAPI
		currentMappings := mappings
		err = h.server.RegisterHandler(
			mcp.NewToolWithRawSchema(toolName, description, json.RawMessage(inputSchema)),
			func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
				return h.buildToolResult(ctx, currentAPI, currentMappings, request)
			},
		)
		if err != nil {
...
func (h *McpHTTPHandler) listBaseAPIs(ctx context.Context, onlyMcpEnabled bool) ([]*models.BaseAPI, error) {
...
// filterTools 根据当前 base_api 的 MCP 开关动态过滤工具列表。
func (h *McpHTTPHandler) filterTools(ctx context.Context, tools []mcp.Tool) []mcp.Tool {
	if len(tools) == 0 {
		return nil
	}
	...
	for _, tool := range tools {
		operation := h.toolOperationMap[tool.Name]
...
// buildToolResult 构建 MCP 工具调用结果。
func (h *McpHTTPHandler) buildToolResult(ctx context.Context, baseAPI *models.BaseAPI, mappings []mcpArgMappingItem, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	var err error
	apiPayload := map[string]any{
		...
		"path":          baseAPI.Path,
		"arg_mapping":   h.parseJSONValue(baseAPI.ArgMapping),
		"output_schema": h.parseJSONValue(baseAPI.OutputSchema),
	}
	...
	req, err = h.buildHTTPRequest(ctx, baseAPI, request.Header, mappings, args)
	if err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
	...
	payload := map[string]any{
		...
		"body":        h.parseJSONValue(string(responseBytes)),
	}
...
func (h *McpHTTPHandler) buildToolDefinition(baseAPI *models.BaseAPI, toolNames map[string]int, openAPISchemas map[string]any) (string, string, string) {
	...
	inputSchema := h.normalizeInputSchema(baseAPI.InputSchema, openAPISchemas)
	return name, description, inputSchema
}

// normalizeHTTPEndpoint 将 HTTP 监听地址转换为 MCP 内部调用地址。
func (h *McpHTTPHandler) normalizeHTTPEndpoint(addr string) string {
...
// newHTTPClient 创建 MCP 内部 HTTP 转发客户端。
func (h *McpHTTPHandler) newHTTPClient(timeout time.Duration) *http.Client {
	if timeout <= 0 {
...
// ensureBaseAPIEnabled 确认工具调用时接口仍处于 MCP 启用状态。
func (h *McpHTTPHandler) ensureBaseAPIEnabled(ctx context.Context, baseAPI *models.BaseAPI) error {
	if baseAPI == nil || baseAPI.ID == 0 {
		return nil
	}
	query := h.baseAPIRepo.Query(ctx).BaseAPI
	opts := make([]repository.QueryOption, 0, 1)
	opts = append(opts, repository.Where(query.ID.Eq(baseAPI.ID)))
	var currentAPI *models.BaseAPI
	var err error
	currentAPI, err = h.baseAPIRepo.Find(ctx, opts...)
	if err != nil {
...
// buildHTTPRequest 根据参数映射构建 MCP 工具转发的 HTTP 请求。
func (h *McpHTTPHandler) buildHTTPRequest(ctx context.Context, baseAPI *models.BaseAPI, sourceHeader http.Header, mappings []mcpArgMappingItem, args map[string]any) (*http.Request, error) {
	...
	for _, mapping := range mappings {
		...
		case mcpArgPositionPath:
			...
			requestPath = strings.ReplaceAll(requestPath, placeholder, url.PathEscape(h.valueToString(value)))
		// query 参数追加到 URL 查询串中。
		case mcpArgPositionQuery:
			switch typedValue := value.(type) {
			case []any:
				for _, item := range typedValue {
					queryValues.Add(mapping.Name, h.valueToString(item))
				}
			case []string:
				for _, item := range typedValue {
			...
			default:
				queryValues.Add(mapping.Name, h.valueToString(value))
			}
	...
	var err error
	var baseURL *url.URL
	baseURL, err = url.Parse(h.httpEndpoint)
	if err != nil {
		return nil, err
	}
	...
	for _, mapping := range mappings {
		...
		req.Header.Set(mapping.Name, h.valueToString(value))
	}
	return req, nil
}

// valueToString 将工具参数转换为 HTTP 参数字符串。
func (h *McpHTTPHandler) valueToString(value any) string {
	switch typedValue := value.(type) {
	case string:
		return typedValue
	case fmt.Stringer:
...
func (h *McpHTTPHandler) parseJSONValue(value string) any {
...
// loadOpenAPISchemas 从内嵌 OpenAPI 文档中读取组件 Schema。
func (h *McpHTTPHandler) loadOpenAPISchemas() map[string]any {
	var document map[string]any
	if err := yaml.Unmarshal(assets.OpenAPIData, &document); err != nil {
		log.Errorf("解析 OpenAPI 文档失败 err=%v", err)
		return nil
	}
	components, ok := h.stringMapValue(document["components"])
	if !ok {
		return nil
	}
	schemas, ok := h.stringMapValue(components["schemas"])
	if !ok {
		return nil
	}
	return h.normalizeSchemaMap(schemas)
}

// normalizeInputSchema 生成 MCP 可独立解析的入参 Schema。
func (h *McpHTTPHandler) normalizeInputSchema(inputSchema string, openAPISchemas map[string]any) string {
	...
	if len(openAPISchemas) > 0 {
		schema = h.expandSchemaRefs(schema, openAPISchemas, make(map[string]bool), 0)
	}
	var data []byte
...
// expandSchemaRefs 展开 OpenAPI components/schemas 引用，避免 MCP 客户端无法解析外部引用。
func (h *McpHTTPHandler) expandSchemaRefs(value any, openAPISchemas map[string]any, refStack map[string]bool, depth int) any {
	...
	case map[string]any:
		if ref, ok := typedValue["$ref"].(string); ok {
			...
			if found {
				...
				if !exists || refStack[schemaName] {
					return h.normalizeSchemaValue(typedValue)
				}
				refStack[schemaName] = true
				expanded := h.expandSchemaRefs(h.normalizeSchemaValue(schema), openAPISchemas, refStack, depth+1)
				...
				for key, item := range typedValue {
					...
					expandedMap[key] = h.expandSchemaRefs(item, openAPISchemas, refStack, depth+1)
				}
				return expandedMap
			}
		}
		...
		for key, item := range typedValue {
			result[key] = h.expandSchemaRefs(item, openAPISchemas, refStack, depth+1)
		}
		return result
	case []any:
		result := make([]any, 0, len(typedValue))
		for _, item := range typedValue {
			result = append(result, h.expandSchemaRefs(item, openAPISchemas, refStack, depth+1))
		}
		return result
	default:
		return typedValue
...
// normalizeSchemaMap 递归规范化 OpenAPI YAML 解码后的 Schema。
func (h *McpHTTPHandler) normalizeSchemaMap(value map[string]any) map[string]any {
	result := make(map[string]any, len(value))
	for key, item := range value {
		result[key] = h.normalizeSchemaValue(item)
	}
	return result
}

// normalizeSchemaValue 将 YAML map 转换为 JSON Schema 更易处理的 map[string]any。
func (h *McpHTTPHandler) normalizeSchemaValue(value any) any {
	switch typedValue := value.(type) {
	case map[string]any:
		return h.normalizeSchemaMap(typedValue)
	case map[any]any:
		result := make(map[string]any, len(typedValue))
		for key, item := range typedValue {
			result[fmt.Sprint(key)] = h.normalizeSchemaValue(item)
		}
		return result
	case []any:
		result := make([]any, 0, len(typedValue))
		for _, item := range typedValue {
			result = append(result, h.normalizeSchemaValue(item))
		}
		return result
	default:
		return typedValue
...
func (h *McpHTTPHandler) stringMapValue(value any) (map[string]any, bool) {
	switch typedValue := value.(type) {
	case map[string]any:
