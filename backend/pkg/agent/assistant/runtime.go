...
func (r *Runtime) RunStream(ctx context.Context, input RuntimeInput, onDelta func(string)) (*Response, error) {
	...
	if disabledCall := r.disabledToolCall(ctx, input, toolInfos); disabledCall != nil {
		...
		return r.buildResponse(assistantAgenticMessage(disabledCall.Content), TokenUsage{}, []ToolUsage{disabledCall.Usage}), nil
	}
...
func assistantAgenticMessage(content string) *einoMessage.AgenticMessage {
	return einoMessage.AssistantText(content)
}
