...
// BoolFromInt32 转换数据库布尔值。
func BoolFromInt32(value int32) bool {
	return value == 1
}

// DatabaseColumnLength 解析数据库字段长度。
func DatabaseColumnLength(item ColumnInfo) int32 {
	if item.CharacterMaximumLength.Valid {
		if item.CharacterMaximumLength.Int64 > 2147483647 {
			return 0
		}
		return int32(item.CharacterMaximumLength.Int64)
	}
	if item.NumericPrecision.Valid {
		return int32(item.NumericPrecision.Int64)
	}
	return 0
}

// DatabaseColumnScale 解析数据库数值字段小数位。
func DatabaseColumnScale(item ColumnInfo) int32 {
	if item.NumericScale.Valid {
		return int32(item.NumericScale.Int64)
	}
	return 0
}

// DefaultQueryColumn 判断字段是否默认作为查询条件。
func DefaultQueryColumn(columnName string) bool {
	return columnName == "name" || columnName == "title" || columnName == "code" || DefaultStatusColumn(columnName)
}

// DefaultListColumn 判断字段是否默认展示在列表中。
func DefaultListColumn(columnName string) bool {
	return !isManagedAuditColumn(columnName)
}

// DefaultFormColumn 判断字段是否默认进入表单。
func DefaultFormColumn(columnName string, isPrimary bool, isAutoIncrement bool) bool {
...
// DefaultQueryOperator 根据数据库字段类型推断查询方式。
func DefaultQueryOperator(dbType string, columnName string) string {
	if isDateTimeDBType(dbType) {
		return "between"
	}
	if isBoolDBType(dbType) || isNumericDBType(dbType) || strings.Contains(strings.ToLower(dbType), "enum") || DefaultStatusColumn(columnName) {
		return "eq"
	}
	return "like"
}

// DefaultStatusColumn 判断字段是否默认按启用/禁用状态处理。
func DefaultStatusColumn(columnName string) bool {
	return columnName == "status" || columnName == "state" || strings.HasSuffix(columnName, "_status") || strings.HasSuffix(columnName, "_state")
}

// DefaultQueryComponent 推断查询组件。
func DefaultQueryComponent(dbType string) string {
...
// DefaultListComponent 推断列表展示组件。
func DefaultListComponent(dbType string) string {
	if isDateTimeDBType(dbType) {
		return "date"
	}
	return "text"
...
// DefaultFormComponent 推断表单组件。
func DefaultFormComponent(dbType string) string {
	if isBoolDBType(dbType) {
		return "switch"
	}
	if isNumericDBType(dbType) {
	...
	if strings.Contains(strings.ToLower(dbType), "text") {
		return "textarea"
	}
	return "input"
}
