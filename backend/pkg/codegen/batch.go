...
func isBatchMergeableFile(path string) bool {
	...
	if strings.HasPrefix(path, "backend/service/admin/biz/") && strings.HasSuffix(path, ".go") {
		return true
	}
	if strings.HasPrefix(path, "backend/service/admin/") && strings.HasSuffix(path, "_service.go") {
		return true
	}
	if strings.HasPrefix(path, "frontend/admin/src/api/admin/") && strings.HasSuffix(path, ".ts") {
		return true
	}
	switch path {
	case "backend/service/admin/init.go", "backend/server/services.go", "backend/server/http.go", "backend/server/grpc.go", "backend/server/mcp.go":
		return true
	default:
		return false
	}
