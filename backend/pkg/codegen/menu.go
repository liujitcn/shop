...
func MenuSpecs(table *Table, columns []*CodeGenColumn, methods []*Proto, resourcePath string, tableComment string) (CodeGenMenuSpec, []CodeGenMenuSpec) {
	...
	if listMethod := methodByName[listMethodName]; listMethod != nil {
		pageAPIs = append(pageAPIs, GeneratedRPCPath(listMethod))
	}
	for _, method := range methods {
		if IsOptionProtoMethod(method) {
			pageAPIs = append(pageAPIs, GeneratedRPCPath(method))
		}
	}
	...
	if createMethod := methodByName["Create"+table.EntityName]; createMethod != nil {
		buttonSpecs = append(buttonSpecs, newButtonMenuSpec(permission+":create", "新增"+table.BusinessName, int32(len(buttonSpecs)+1), GeneratedRPCPath(createMethod)))
	}
	...
	if getMethod != nil && updateMethod != nil {
		buttonSpecs = append(buttonSpecs, newButtonMenuSpec(permission+":update", "编辑"+table.BusinessName, int32(len(buttonSpecs)+1), GeneratedRPCPath(getMethod), GeneratedRPCPath(updateMethod)))
	}
	if deleteMethod := methodByName["Delete"+table.EntityName]; deleteMethod != nil {
		buttonSpecs = append(buttonSpecs, newButtonMenuSpec(permission+":delete", "删除"+table.BusinessName, int32(len(buttonSpecs)+1), GeneratedRPCPath(deleteMethod)))
	}
...
func GeneratedRPCPath(method *Proto) string {
	return "/admin.v1." + method.TargetEntityName + "Service/" + method.MethodName
}
