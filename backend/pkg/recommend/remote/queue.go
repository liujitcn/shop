...
func (r *QueueReceiver) consumeRecommendEvent(message queueData.Message) error {
	...
	for _, item := range *eventList {
		...
		if item == nil || item.ActorID <= 0 || item.ActorType != int32(common.RecommendActorType_USER_ACTOR) || item.GoodsID <= 0 || item.EventType == int32(common.RecommendEventType_UNKNOWN_RET) {
			continue
		}
