...
type GoodsCategory struct {
	...
	ParentID  int64          `gorm:"column:parent_id;type:bigint;not null;index:idx_goods_category_parent_id,priority:1;comment:父级商品分类ID" json:"parent_id"` // 父级商品分类ID
	Path      string         `gorm:"column:path;type:varchar(255);not null;comment:商品分类路径" json:"path"`                                                     // 商品分类路径
	Picture   string         `gorm:"column:picture;type:varchar(1024);comment:商品分类图片" json:"picture"`                                                       // 商品分类图片
	Name      string         `gorm:"column:name;type:varchar(255);not null;comment:商品分类名称" json:"name"`                                                     // 商品分类名称
