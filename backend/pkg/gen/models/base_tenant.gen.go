// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.

package models

import (
	"time"

	"gorm.io/gorm"
)

const TableNameBaseTenant = "base_tenant"

// BaseTenant 租户信息表
type BaseTenant struct {
	ID           int64          `gorm:"column:id;type:bigint;primaryKey;autoIncrement:true;comment:租户ID" json:"id"`                                       // 租户ID
	Code         string         `gorm:"column:code;type:varchar(50);uniqueIndex:unique_base_tenant,priority:1;comment:租户编码" json:"code"`                  // 租户编码
	Name         string         `gorm:"column:name;type:varchar(100);comment:租户名称" json:"name"`                                                           // 租户名称
	ContactName  string         `gorm:"column:contact_name;type:varchar(50);comment:联系人" json:"contact_name"`                                             // 联系人
	ContactPhone string         `gorm:"column:contact_phone;type:varchar(20);comment:联系电话" json:"contact_phone"`                                          // 联系电话
	Status       int32          `gorm:"column:status;type:tinyint;index:idx_base_tenant_status,priority:1;default:1;comment:状态：枚举【Status】" json:"status"` // 状态：枚举【Status】
	Remark       string         `gorm:"column:remark;type:varchar(500);comment:备注" json:"remark"`                                                         // 备注
	CreatedBy    int64          `gorm:"column:created_by;type:bigint;comment:创建者ID" json:"created_by"`                                                    // 创建者ID
	UpdatedBy    int64          `gorm:"column:updated_by;type:bigint;comment:更新者ID" json:"updated_by"`                                                    // 更新者ID
	CreatedAt    time.Time      `gorm:"column:created_at;type:datetime;comment:创建时间" json:"created_at"`                                                   // 创建时间
	UpdatedAt    time.Time      `gorm:"column:updated_at;type:datetime;comment:更新时间" json:"updated_at"`                                                   // 更新时间
	DeletedAt    gorm.DeletedAt `gorm:"column:deleted_at;type:datetime;comment:删除时间" json:"deleted_at"`                                                   // 删除时间
}

// TableName BaseTenant's table name
func (*BaseTenant) TableName() string {
	return TableNameBaseTenant
}
