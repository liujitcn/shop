...
type CodeGenColumn struct {
	...
	TableID       int64                 `gorm:"column:table_id;type:bigint;not null;uniqueIndex:unique_code_gen_column,priority:1;index:idx_code_gen_column_table_id,priority:1;index:idx_code_gen_column_table_id_column_name,priority:1;comment:生成对象ID" json:"table_id"` // 生成对象ID
	ColumnName    string                `gorm:"column:column_name;type:varchar(128);not null;uniqueIndex:unique_code_gen_column,priority:2;index:idx_code_gen_column_table_id_column_name,priority:2;comment:字段名" json:"column_name"`                                      // 字段名
	ColumnComment string                `gorm:"column:column_comment;type:varchar(255);comment:字段描述" json:"column_comment"`                                                                                                                                                // 字段描述
	...
	DeletedAt     soft_delete.DeletedAt `gorm:"column:deleted_at;type:bigint unsigned;not null;uniqueIndex:unique_code_gen_column,priority:3;softDelete:milli" json:"deleted_at"`
}
