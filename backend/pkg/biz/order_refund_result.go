...
func (c *OrderRefundResultCase) Apply(ctx context.Context, orderTrade *models.OrderTrade, refundResource *appv1.RefundResource) (bool, error) {
	if orderTrade == nil {
	...
	if refundResource.GetRefundStatus() == appv1.RefundResource_SUCCESS {
...
func (c *OrderRefundResultCase) FailPending(ctx context.Context, orderRefund *models.OrderRefund) (bool, error) {
	...
	err := c.tx.Transaction(ctx, func(ctx context.Context) error {
		...
		result, updateErr := query.WithContext(ctx).
			Where(query.ID.Eq(orderRefund.ID), query.RefundState.Eq(appv1.RefundResource_PROCESSING.String())).
			Update(query.RefundState, appv1.RefundResource_ABNORMAL.String())
		if updateErr != nil {
			return updateErr
		}
		if result.RowsAffected == 0 {
...
func validateOrderRefundResult(orderTrade *models.OrderTrade, orderRefund *models.OrderRefund, refundResource *appv1.RefundResource) error {
	switch refundResource.GetRefundStatus() {
	case appv1.RefundResource_SUCCESS, appv1.RefundResource_CLOSED, appv1.RefundResource_PROCESSING, appv1.RefundResource_ABNORMAL:
	default:
...
func shouldApplyOrderRefundResult(current string, next appv1.RefundResource_RefundStatus) bool {
	return slices.Contains(orderRefundResultSourceStates(next), current)
}

// orderRefundResultSourceStates 返回指定渠道结果允许覆盖的本地退款状态。
func orderRefundResultSourceStates(next appv1.RefundResource_RefundStatus) []string {
	switch next {
	case appv1.RefundResource_SUCCESS:
		return []string{
			appv1.RefundResource_PROCESSING.String(),
			appv1.RefundResource_CLOSED.String(),
			appv1.RefundResource_ABNORMAL.String(),
		}
	case appv1.RefundResource_CLOSED, appv1.RefundResource_PROCESSING, appv1.RefundResource_ABNORMAL:
		return []string{appv1.RefundResource_PROCESSING.String()}
	default:
		return nil
	}
}
