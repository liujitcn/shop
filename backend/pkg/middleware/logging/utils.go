...
func getClientRealIP(request *http.Request) string {
	...
	// 当请求经过多个代理服务器时，X-Forwarded-For字段可以完整地记录原始请求的客户端 IP地址和所有代理服务器的IP地址。
	// 需要注意：
	...
	if xff != "" {
		...
		// 一般来说，第一个IP地址是原始请求的客户端 IP地址（当然，它可以被伪造）。
	...
	// 通常只在反向代理服务器中使用，并且只记录原始请求的客户端 IP地址。
	// 它不适用于多级代理的情况，因为每经过一个代理服务器，X-Real-IP字段的值都会被覆盖为最新的客户端 IP地址。
