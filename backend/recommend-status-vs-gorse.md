...
- 缺少“最终推荐结果”的离线预计算与按用户直出能力
- 缺少真正生效的实验分流与灰度放量能力
- 缺少推荐专用控制台、运行面板和缓存巡检能力
- 缺少更通用的内容表示、外部推荐器接入和多节点工程形态
...
| 最终推荐结果离线预生成 | 缓存协议预留，但未形成稳定任务 | Worker 批量生成推荐结果 | 当前项目明显缺失 |
| 推荐在线服务形态 | 单体服务内动态组召回和排序 | 独立 server + worker + master | 当前项目平台化不足 |
| 灰度实验/A-B | 只有配置字段，运行时未真实分流 | 更偏平台化实验与替换能力 | 当前项目明显缺失 |
...
当前代码中，缓存协议已经预留了 `recommend` 集合：

- `backend/pkg/recommend/cache/types.go`

但当前 `Materializer` 只发布这些结果：
...
这和 gorse 的最大差异，不在于“会不会召回”，而在于“有没有把最终推荐结果批量提前算好”。

### 二：`grayRatio` 目前更像配置项，而不是运行时能力
...
这说明当前系统已经具备“版本配置模型”，但还没有进入“实验系统模型”。
