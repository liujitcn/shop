...
- 推荐能力：推荐请求与事件先落本地表，再异步投递到 Gorse，并通过定时任务全量同步登录用户与商品快照；管理端提供本地推荐请求排查页，可串联 `recommend_request` / `recommend_request_item` / `recommend_event` 还原请求链路、命中推荐器、商品结果和事件明细；远程推荐页面继续通过后端薄代理调用远程推荐原生接口，不在本地保存远程用户、商品和编排配置副本。
