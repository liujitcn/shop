...
`CodeGenService` 与 `CodeGenCase` 在只读加载上述现有配置后提供代码预览、单项或批量生成和任务进度查询。预览可临时覆盖本次单项任务的仓库内输出路径，但不会回写配置；批量任务按各表默认路径串行写入，使用服务当前生效的数据源逐表生成 GORM 代码，并在整批末尾只执行一次接口产物、Wire 和格式化命令。外部选项目标已有的 Biz、Service 和前端 API 方法保持不变，只补齐缺失方法；已有前端页面无法确认由生成器管理时，预览继续标记跳过，实际生成会在任何文件写入前取消整个批次，避免只覆盖接口与后端。生成任务保存在进程内并按用户隔离，通过任务 ID 的 SSE 通道推送实时进度，同时支持查询接口轮询恢复。生成流程不会修改代码生成表的状态或备注；启用菜单同步且前端页面所需 RPC 完整时，只同步目标业务页面及实际存在的按钮接口权限。
