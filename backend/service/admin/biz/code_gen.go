...
func (c *CodeGenCase) PreviewCodeGen(ctx context.Context, tableID int64, requestedPaths *adminv1.CodeGenOutputPaths) (*adminv1.PreviewCodeGenResponse, error) {
	...
	return &adminv1.PreviewCodeGenResponse{Files: generation.Files, OutputPaths: generation.OutputPaths}, nil
}
...
func (c *CodeGenCase) runCodeGenTask(
	...
	for _, tableID := range tableIDs {
		...
		if codegen.ShouldSyncMenus(generation.Table, generation.GeneratedMethods) {
			reporter.updateStep(workflowCtx, codegen.MenuStepID, adminv1.CodeGenTaskStepStatus_CODE_GEN_TASK_STEP_STATUS_RUNNING, "正在同步", "")
			...
			if err != nil {
				failedCount++
				reporter.updateStep(workflowCtx, codegen.MenuStepID, adminv1.CodeGenTaskStepStatus_CODE_GEN_TASK_STEP_STATUS_FAILED, err.Error(), "")
				c.progressManager.MarkTableCompleted(ctx, taskID, tableID, adminv1.CodeGenTaskStatus_CODE_GEN_TASK_STATUS_FAILED, codegen.FailureRemark(err))
				continue
			}
			reporter.updateStep(workflowCtx, codegen.MenuStepID, adminv1.CodeGenTaskStepStatus_CODE_GEN_TASK_STEP_STATUS_SUCCEEDED, "同步完成", "")
		}
...
func (c *CodeGenCase) prepareCodeGenBatch(ctx context.Context, tableIDs []int64, requestedPaths *adminv1.CodeGenOutputPaths) (*codeGenBatchContext, error) {
...
func (c *CodeGenCase) validateCodeGenParentMenu(ctx context.Context, parentMenuID int64) error {
	...
	if menu.Type != _const.BASE_MENU_TYPE_FOLDER && menu.Type != _const.BASE_MENU_TYPE_MENU {
		return errorsx.InvalidArgument("父级菜单只能选择目录或菜单")
	}
	return nil
}
...
func (c *CodeGenCase) disableStaleGeneratedStatusMenus(ctx context.Context, pageMenuID int64, table *codegen.Table, buttonSpecs []codegen.CodeGenMenuSpec) error {
	...
	statusAPIPrefix := "/admin.v1." + table.EntityName + "Service/Set"
	for _, menu := range menus {
...
func codeGenProtosToSnapshots(items []*models.CodeGenProto) ([]*codegen.Proto, error) {
	...
	for _, item := range items {
		var config adminv1.CodeGenProtoConfig
		if item.Config != "" {
