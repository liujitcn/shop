...
func (c *renderer) newAdminRegistrationPreviewFiles(table *Table, methods []*Proto) []*systemadminv1.CodeGenPreviewFile {
	...
	patches := []struct {
		...
		{path: filepath.ToSlash(filepath.Join(protoTarget.BackendModuleDirectory, "init.go")), patch: appendAdminProviderRegistration},
		{path: protoTarget.ModuleRegisterPath, patch: appendProtoServiceRegistration},
	}
	files := make([]*systemadminv1.CodeGenPreviewFile, 0, len(patches))
	for _, item := range patches {
		patch := item.patch
		files = append(files, c.newExistingPatchedPreviewFile(item.path, func(content string) string {
			for _, entity := range entities {
				content = patch(content, protoTarget, entity)
			}
			return content
		}))
	}
