...
func GenerateAdminReply(
	...
	reply, err := runner.handleAdminFlowAction(ctx, &basev1.AiAction{
		Flow: flow,
...
func (r *AdminRunner) openAdminPendingShipmentFlow(ctx context.Context) (*ai.Response, error) {
	output, usage, err := r.invokeAdminFlowTool(ctx, adminToolPageOrderInfo, map[string]any{
		"status":    int(shopcommonv1.OrderInfoStatus_WAIT_SHIPMENT_OIS),
		"page_num":  1,
		"page_size": 5,
	})
...
func (r *AdminRunner) viewAdminShipmentDetail(ctx context.Context, payload map[string]any) (*ai.Response, error) {
...
func (r *AdminRunner) confirmAdminShipment(ctx context.Context, payload map[string]any) (*ai.Response, error) {
	orderID := int64Value(payload["order_id"])
	if orderID <= 0 {
		return nil, errorsx.InvalidArgument("订单参数不合法")
	}
...
func (r *AdminRunner) openAdminCommentReviewFlow(ctx context.Context) (*ai.Response, error) {
	output, usage, err := r.invokeAdminFlowTool(ctx, adminToolPageCommentInfo, map[string]any{
		"status":    int(shopcommonv1.CommentStatus_PENDING_REVIEW_CS),
		"page_num":  1,
		"page_size": 5,
	})
	tools := appendAiFlowTool(nil, usage)
...
func (r *AdminRunner) viewAdminCommentDetail(ctx context.Context, payload map[string]any) (*ai.Response, error) {
...
func (r *AdminRunner) confirmAdminCommentReview(ctx context.Context, payload map[string]any) (*ai.Response, error) {
	commentID := int64Value(payload["comment_id"])
	if commentID <= 0 {
		commentID = int64Value(payload["id"])
	}
...
func (r *AdminRunner) openAdminGoodsInventoryAlertFlow(ctx context.Context) (*ai.Response, error) {
	output, usage, err := r.invokeAdminFlowTool(ctx, adminToolPageGoodsInfo, map[string]any{
		"inventory_alert": int(shopcommonv1.GoodsInventoryAlert_LOW_STOCK),
		"page_num":        1,
		"page_size":       5,
	})
	tools := appendAiFlowTool(nil, usage)
...
func (r *AdminRunner) viewAdminGoodsDetail(ctx context.Context, payload map[string]any) (*ai.Response, error) {
...
func (r *AdminRunner) confirmAdminGoodsStatus(ctx context.Context, payload map[string]any) (*ai.Response, error) {
	goodsID := int64Value(payload["goods_id"])
	if goodsID <= 0 {
		goodsID = int64Value(payload["id"])
	}
	...
	status := int64Value(payload["status"])
	if status <= 0 {
		status = int64(shopcommonv1.GoodsStatus_PULL_OFF)
	}
...
func (r *AdminRunner) openAdminOrderRefundFlow(ctx context.Context) (*ai.Response, error) {
	refundStatuses := []shopcommonv1.OrderRefundStatus{
		shopcommonv1.OrderRefundStatus_PROCESSING_ORS,
		shopcommonv1.OrderRefundStatus_PARTIAL_REFUND_ORS,
		shopcommonv1.OrderRefundStatus_REFUNDED_ORS,
		shopcommonv1.OrderRefundStatus_CLOSED_OR_FAILED_ORS,
	}
	tools := make([]ai.ToolUsage, 0, len(refundStatuses))
	orderInfos := make([]any, 0, len(refundStatuses)*5)
	var total int64
	var err error
	for _, refundStatus := range refundStatuses {
		var output map[string]any
		var usage ai.ToolUsage
		output, usage, err = r.invokeAdminFlowTool(ctx, adminToolPageOrderInfo, map[string]any{
...
func (r *AdminRunner) openAdminGoodsAnalyticsFlow(ctx context.Context) (*ai.Response, error) {
	tools := make([]ai.ToolUsage, 0, 4)
...
func (r *AdminRunner) openAdminOrderAnalyticsFlow(ctx context.Context) (*ai.Response, error) {
	tools := make([]ai.ToolUsage, 0, 3)
	timeType := int(commonv1.AnalyticsTimeType_ANALYTICS_TIME_TYPE_WEEK)
...
func (r *AdminRunner) openAdminStoreAuditFlow(ctx context.Context) (*ai.Response, error) {
	output, usage, err := r.invokeAdminFlowTool(ctx, adminToolPageUserStore, map[string]any{
		"status":    int(shopcommonv1.UserStoreStatus_PENDING_REVIEW),
		"page_num":  1,
		"page_size": 5,
	})
	tools := appendAiFlowTool(nil, usage)
...
func (r *AdminRunner) openAdminPayBillCheckFlow(ctx context.Context) (*ai.Response, error) {
	output, usage, err := r.invokeAdminFlowTool(ctx, adminToolPagePayBill, map[string]any{
		"status":    int(shopcommonv1.PayBillStatus_HAS_ERROR),
		"page_num":  1,
		"page_size": 5,
	})
	tools := appendAiFlowTool(nil, usage)
...
func (r *AdminRunner) openAdminReportOverviewFlow(ctx context.Context, payload map[string]any) (*ai.Response, error) {
	tools := make([]ai.ToolUsage, 0, 3)
	...
	tools = appendAiFlowTool(tools, goodsUsage)
	...
	tools = appendAiFlowTool(tools, userUsage)
...
func (r *AdminRunner) invokeAdminFlowTool(ctx context.Context, name string, input map[string]any) (map[string]any, ai.ToolUsage, error) {
...
func buildAdminOrderListBlock(flow string, title string, output map[string]any, actionType string) map[string]any {
	...
	for _, item := range orders {
		...
		action := aiAction(flow, "detail", actionType, payload)
...
func buildAdminShipmentFormBlock(output map[string]any, orderID int64, canShip bool) map[string]any {
	...
	if canShip {
		block["action"] = aiAction(adminFlowPendingShipment, "confirm", "confirm_shipment", map[string]any{
			"order_id": orderID,
		})
	}
	return block
...
func buildAdminCommentListBlock(flow string, output map[string]any) map[string]any {
	...
	for _, item := range comments {
		...
		items = append(items, map[string]any{
			...
			"created_at":   item["created_at"],
			"action":       aiAction(flow, "detail", "view_comment_detail", payload),
		})
	}
	return map[string]any{
		"type":     "comment_list",
...
func buildAdminCommentDetailBlock(flow string, output map[string]any) map[string]any {
	...
	return map[string]any{
		...
		"actions": []map[string]any{
			aiAction(flow, "confirm", "confirm_comment_review", map[string]any{
				"comment_id": commentID,
				"status":     int(shopcommonv1.CommentStatus_APPROVED_CS),
				"reason":     "",
			}),
			aiAction(flow, "confirm", "confirm_comment_review", map[string]any{
				"comment_id": commentID,
				"status":     int(shopcommonv1.CommentStatus_REJECTED_CS),
				"reason":     "",
			}),
		},
	}
...
func buildAdminGoodsAlertListBlock(flow string, output map[string]any) map[string]any {
	...
	for _, item := range goods {
		...
		items = append(items, map[string]any{
			...
			"action":          aiAction(flow, "detail", "view_goods_detail", payload),
		})
	}
	return map[string]any{
...
func buildAdminGoodsAlertDetailBlock(flow string, output map[string]any) map[string]any {
	...
	return map[string]any{
		...
		"goods": goods,
		"actions": []map[string]any{
			aiAction(flow, "confirm", "confirm_goods_status", map[string]any{
				"goods_id": goodsID,
				"status":   int(shopcommonv1.GoodsStatus_PULL_OFF),
			}),
		},
	}
}
...
func buildAdminStoreAuditListBlock(flow string, output map[string]any) map[string]any {
	...
	for _, item := range stores {
		...
		items = append(items, map[string]any{
			...
			"remark":       item["remark"],
			"action":       aiAction(flow, "detail", "view_store_detail", payload),
		})
	}
	return map[string]any{
...
func buildAdminStoreDetailBlock(flow string, output map[string]any) map[string]any {
	...
	return map[string]any{
		...
		"actions": []map[string]any{
			aiAction(flow, "confirm", "confirm_store_audit", map[string]any{
				"store_id": storeID,
				"status":   int(shopcommonv1.UserStoreStatus_APPROVED),
				"remark":   "",
			}),
			aiAction(flow, "confirm", "confirm_store_audit", map[string]any{
				"store_id": storeID,
				"status":   int(shopcommonv1.UserStoreStatus_FAILED_REVIEW),
				"remark":   "",
			}),
		},
	}
