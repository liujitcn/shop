...
func DispatchRecommendSyncBaseUser(userID int64) {
	...
	corequeue.AddQueue(_const.RECOMMEND_SYNC_BASE_USER, userID)
}
...
func DispatchRecommendDeleteBaseUser(userIDs []int64) {
	...
	corequeue.AddQueue(_const.RECOMMEND_DELETE_BASE_USER, userIDs)
}
...
func DispatchRecommendSyncGoodsInfo(goodsID int64) {
	...
	corequeue.AddQueue(_const.RECOMMEND_SYNC_GOODS_INFO, goodsID)
}
...
func DispatchRecommendDeleteGoodsInfo(goodsIDs []int64) {
	...
	corequeue.AddQueue(_const.RECOMMEND_DELETE_GOODS_INFO, goodsIDs)
}
...
func DispatchRecommendEventList(eventList []*models.RecommendEvent) {
	...
	corequeue.AddQueue(_const.RECOMMEND_EVENT, eventList)
}
