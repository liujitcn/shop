...
type RecommendEventReportEvent struct {
	...
	EventType      shopcommonv1.RecommendEventType // 推荐事件类型
	Scene          int32                           // 推荐场景
...
func DispatchRecommendEvent(actor *dto.RecommendActor, req *shopappv1.RecommendEventReportRequest, eventTime time.Time) {
	...
	if eventType == shopcommonv1.RecommendEventType(_const.RECOMMEND_EVENT_TYPE_UNKNOWN) {
		return
	}
	...
	corequeue.AddQueue(_const.RECOMMEND_EVENT_REPORT, &RecommendEventReportEvent{
