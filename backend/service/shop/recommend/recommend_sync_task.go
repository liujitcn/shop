...
type RecommendSync struct {
	...
	userSync      *gorse.UserSyncReceiver
	goodsSync     *gorse.GoodsSyncReceiver
	ctx           context.Context
}

var _ job.TaskExec = (*RecommendSync)(nil)
...
func (t *RecommendSync) Exec(args map[string]string) ([]string, error) {
	...
	if rawBatchSize != "" {
		...
		if parsedBatchSize <= 0 {
			argumentErr := errorsx.InvalidArgument("batchSize 必须大于 0")
			return []string{argumentErr.Error()}, argumentErr
		}
