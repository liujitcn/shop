...
func (r *ChainReceiver) ExecutePlan(
	ctx context.Context,
	scene shopcommonv1.RecommendScene,
	...
	result := &dto.GoodsResult{
		GoodsIDs: []int64{},
		Strategy: shopcommonv1.RecommendStrategy(_const.RECOMMEND_STRATEGY_REMOTE),
...
func (r *ChainReceiver) buildRecommendChain(scene shopcommonv1.RecommendScene, actor *dto.RecommendActor) []ProviderName {
	isLogin := actor.IsUser()
	steps := make([]ProviderName, 0, 6)
	switch scene {
	case shopcommonv1.RecommendScene(_const.RECOMMEND_SCENE_HOME):
		// 首页
	...
	case shopcommonv1.RecommendScene(_const.RECOMMEND_SCENE_PROFILE):
		...
		steps = append(steps, NON_PERSONALIZED_HOT_30D)
		steps = append(steps, LATEST)
	case shopcommonv1.RecommendScene(_const.RECOMMEND_SCENE_ORDER_DETAIL):
		// 订单详情
		...
		steps = append(steps, NON_PERSONALIZED_HOT_30D)
		steps = append(steps, LATEST)
	case shopcommonv1.RecommendScene(_const.RECOMMEND_SCENE_ORDER_PAID):
		// 支付成功页
