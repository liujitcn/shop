...
type OrderLogisticsCase struct {
	...
	*data.OrderLogisticsRepository
	mapper *mapper.CopierMapper[shopadminv1.OrderLogistics, models.OrderLogistics]
}
...
func NewOrderLogisticsCase(baseCase *biz.BaseCase, orderLogisticsRepo *data.OrderLogisticsRepository) *OrderLogisticsCase {
	orderLogisticsMapper := mapper.NewCopierMapper[shopadminv1.OrderLogistics, models.OrderLogistics]()
	orderLogisticsMapper.AppendConverters(mapper.NewJSONTypeConverter[[]*shopadminv1.OrderLogistics_Detail]().NewConverterPair())
...
func (c *OrderLogisticsCase) FindFromByOrderID(ctx context.Context, orderID int64) (*shopadminv1.OrderLogistics, error) {
	...
	if err != nil {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return &shopadminv1.OrderLogistics{}, nil
		}
		// 其他查询错误直接返回，避免吞掉真实异常。
		return nil, err
	}
