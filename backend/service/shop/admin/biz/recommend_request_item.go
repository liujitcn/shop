...
func (c *RecommendRequestItemCase) ListRecommendRequestItems(
...
) ([]*shopadminv1.RecommendRequestItem, error) {
	resList := make([]*shopadminv1.RecommendRequestItem, 0)
	...
	for _, item := range itemList {
		...
		requestItem := &shopadminv1.RecommendRequestItem{
		...
		if goodsInfo != nil {
			...
			requestItem.GoodsStatus = shopcommonv1.GoodsStatus(goodsInfo.Status)
		}
