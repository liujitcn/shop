...
type CommentTagCase struct {
	...
	*data.CommentTagRepository
	commentTagMapper *mapper.CopierMapper[shopadminv1.CommentTag, models.CommentTag]
}
...
func NewCommentTagCase(baseCase *biz.BaseCase, commentTagRepo *data.CommentTagRepository) *CommentTagCase {
	return &CommentTagCase{
		...
		commentTagMapper:     mapper.NewCopierMapper[shopadminv1.CommentTag, models.CommentTag](),
	}
}

// ListByGoodsID 查询商品评论标签。
func (c *CommentTagCase) ListByGoodsID(ctx context.Context, goodsID int64) ([]*shopadminv1.CommentTag, error) {
	...
	resList := make([]*shopadminv1.CommentTag, 0, len(list))
