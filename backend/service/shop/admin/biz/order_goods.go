...
type OrderGoodsCase struct {
	...
	*data.OrderGoodsRepository
	mapper *mapper.CopierMapper[shopadminv1.OrderGoods, models.OrderGoods]
}
...
func NewOrderGoodsCase(baseCase *biz.BaseCase, orderGoodsRepo *data.OrderGoodsRepository) *OrderGoodsCase {
	return &OrderGoodsCase{
		...
		mapper:               mapper.NewCopierMapper[shopadminv1.OrderGoods, models.OrderGoods](),
	}
}

// FindFromByOrderID 按订单查询商品信息
func (c *OrderGoodsCase) FindFromByOrderID(ctx context.Context, orderID int64) ([]*shopadminv1.OrderGoods, error) {
	...
	res := make([]*shopadminv1.OrderGoods, 0, len(list))
