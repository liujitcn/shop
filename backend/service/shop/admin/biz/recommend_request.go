...
func (c *RecommendRequestCase) PageRecommendRequest(ctx context.Context, req *shopadminv1.PageRecommendRequestRequest) (*shopadminv1.PageRecommendRequestResponse, error) {
	...
	if req.ActorType != nil && req.GetActorType() > shopcommonv1.RecommendActorType(_const.RECOMMEND_ACTOR_TYPE_UNKNOWN) {
	...
	if req.Scene != nil && req.GetScene() != shopcommonv1.RecommendScene(_const.RECOMMEND_SCENE_UNKNOWN) {
	...
	resList := make([]*shopadminv1.RecommendRequest, 0, len(list))
	for _, item := range list {
		resList = append(resList, c.toRecommendRequest(item, actorNameMap[item.ActorID]))
	}
	return &shopadminv1.PageRecommendRequestResponse{
...
func (c *RecommendRequestCase) GetRecommendRequest(ctx context.Context, id int64) (*shopadminv1.RecommendRequestDetailResponse, error) {
	...
	var itemList []*shopadminv1.RecommendRequestItem
	itemList, err = c.recommendRequestItemCase.ListRecommendRequestItems(ctx, requestModel)
	if err != nil {
		return nil, err
	}

	return &shopadminv1.RecommendRequestDetailResponse{
...
func (c *RecommendRequestCase) toRecommendRequest(item *models.RecommendRequest, actorName string) *shopadminv1.RecommendRequest {
	// 请求实体为空时，回退到空响应结构，避免列表渲染空指针。
	if item == nil {
		return &shopadminv1.RecommendRequest{}
	}
...
func (c *RecommendRequestCase) toRecommendRequestContext(
	contextRecord *recommendDto.RecommendContext,
	rawJSON string,
) *shopadminv1.RecommendRequestContext {
	...
	traceList := make([]*shopadminv1.RecommendRequestTrace, 0, len(contextRecord.Trace))
	for _, item := range contextRecord.Trace {
		// 链路节点为空时，直接跳过无效节点。
		if item == nil {
			continue
		}
		traceList = append(traceList, &shopadminv1.RecommendRequestTrace{
	...
	return &shopadminv1.RecommendRequestContext{
