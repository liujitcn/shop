...
type PayBillCase struct {
	...
	*data.PayBillRepository
	mapper *mapper.CopierMapper[shopadminv1.PayBill, models.PayBill]
}
...
func NewPayBillCase(baseCase *biz.BaseCase, payBillRepo *data.PayBillRepository) *PayBillCase {
	return &PayBillCase{
		...
		mapper:            mapper.NewCopierMapper[shopadminv1.PayBill, models.PayBill](),
	}
}

// PagePayBill 查询支付账单列表
func (c *PayBillCase) PagePayBill(ctx context.Context, req *shopadminv1.PagePayBillRequest) (*shopadminv1.PagePayBillResponse, error) {
	...
	resList := make([]*shopadminv1.PayBill, 0, len(list))
	for _, item := range list {
		payBill := c.mapper.ToDTO(item)
		resList = append(resList, payBill)
	}
	return &shopadminv1.PagePayBillResponse{PayBills: resList, Total: int32(total)}, nil
}
