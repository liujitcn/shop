...
type GoodsSpecCase struct {
	...
	*data.GoodsSpecRepository
	mapper *mapper.CopierMapper[shopadminv1.GoodsSpec, models.GoodsSpec]
}
...
func NewGoodsSpecCase(baseCase *biz.BaseCase, goodsSpecRepo *data.GoodsSpecRepository) *GoodsSpecCase {
	goodsSpecMapper := mapper.NewCopierMapper[shopadminv1.GoodsSpec, models.GoodsSpec]()
...
func (c *GoodsSpecCase) ListGoodsSpec(ctx context.Context, req *shopadminv1.ListGoodsSpecRequest) (*shopadminv1.ListGoodsSpecResponse, error) {
	...
	resList := make([]*shopadminv1.GoodsSpec, 0, len(list))
	for _, item := range list {
		resList = append(resList, c.mapper.ToDTO(item))
	}
	return &shopadminv1.ListGoodsSpecResponse{GoodsSpecs: resList}, nil
}
