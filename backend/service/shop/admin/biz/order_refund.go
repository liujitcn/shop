...
type OrderRefundCase struct {
	...
	*data.OrderRefundRepository
	mapper *mapper.CopierMapper[shopadminv1.OrderRefund, models.OrderRefund]
}
...
func NewOrderRefundCase(baseCase *biz.BaseCase, orderRefundRepo *data.OrderRefundRepository) *OrderRefundCase {
	orderRefundMapper := mapper.NewCopierMapper[shopadminv1.OrderRefund, models.OrderRefund]()
	orderRefundMapper.AppendConverters(mapper.NewJSONTypeConverter[*shopadminv1.OrderRefund_Amount]().NewConverterPair())
...
func (c *OrderRefundCase) FindFromByOrderID(ctx context.Context, orderID int64) ([]*shopadminv1.OrderRefund, error) {
	...
	if err != nil {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return []*shopadminv1.OrderRefund{}, nil
		}
		return nil, err
	}

	res := make([]*shopadminv1.OrderRefund, 0, len(list))
