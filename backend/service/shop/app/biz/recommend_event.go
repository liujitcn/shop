...
func (c *RecommendEventCase) saveRecommendEventReport(message queueData.Message) error {
	recommendEvent, err := corequeue.DecodeQueueData[queue.RecommendEventReportEvent](message)
	if err != nil {
		return err
	}
	...
	items := make([]*shopappv1.RecommendEventItem, 0, len(recommendEvent.Items))
	for _, item := range recommendEvent.Items {
		// 非法商品项直接跳过，避免把脏数据写入推荐链路。
		if item == nil || item.GoodsID <= 0 {
			continue
		}
		items = append(items, &shopappv1.RecommendEventItem{
	...
	recommendEventReport := &shopappv1.RecommendEventReportRequest{
		EventType: recommendEvent.EventType,
		RecommendContext: &shopappv1.RecommendEventContext{
			Scene:     shopcommonv1.RecommendScene(recommendEvent.Scene),
