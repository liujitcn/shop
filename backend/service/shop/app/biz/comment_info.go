...
type CommentInfoCase struct {
	...
	*data.CommentInfoRepository
	mapper *mapper.CopierMapper[shopappv1.CommentItem, models.CommentInfo]
}
...
func NewCommentInfoCase(baseCase *biz.BaseCase, commentInfoRepo *data.CommentInfoRepository) *CommentInfoCase {
	return &CommentInfoCase{
		...
		mapper:                mapper.NewCopierMapper[shopappv1.CommentItem, models.CommentInfo](),
	}
}
...
func (c *CommentInfoCase) PageGoodsComment(ctx context.Context, req *shopappv1.PageGoodsCommentRequest, userID int64) ([]*shopappv1.CommentItem, int32, error) {
	...
	for _, record := range recordList {
		...
		// 仅筛选有图评价时，要求图片列表非空。
		if req.GetFilterType() == shopcommonv1.CommentFilterType(_const.COMMENT_FILTER_TYPE_MEDIA) && len(imageList) == 0 {
			continue
		}
		// 好评筛选要求商品评分大于等于 4。
		if req.GetFilterType() == shopcommonv1.CommentFilterType(_const.COMMENT_FILTER_TYPE_GOOD) && record.GoodsScore < 4 {
			continue
		}
		// 中评筛选要求商品评分等于 3。
		if req.GetFilterType() == shopcommonv1.CommentFilterType(_const.COMMENT_FILTER_TYPE_MIDDLE) && record.GoodsScore != 3 {
			continue
		}
		// 差评筛选要求商品评分小于等于 2。
		if req.GetFilterType() == shopcommonv1.CommentFilterType(_const.COMMENT_FILTER_TYPE_BAD) && record.GoodsScore > 2 {
			continue
		}
		// 标签筛选要求传入有效标签并命中评价标签列表。
		if req.GetFilterType() == shopcommonv1.CommentFilterType(_const.COMMENT_FILTER_TYPE_TAG) {
			hasTag := false
	...
	if req.GetSortType() == shopcommonv1.CommentSortType(_const.COMMENT_SORT_TYPE_LATEST) {
...
func (c *CommentInfoCase) PageMyComment(ctx context.Context, userID int64, req *shopappv1.PageMyCommentRequest) ([]*shopappv1.CommentItem, int32, error) {
...
func (c *CommentInfoCase) listPreviewByRecordList(ctx context.Context, recordList []*models.CommentInfo, previewLimit int32, userID int64) ([]*shopappv1.CommentItem, error) {
...
func (c *CommentInfoCase) buildCommentItems(ctx context.Context, recordList []*models.CommentInfo, ownerView bool, userID int64) ([]*shopappv1.CommentItem, error) {
	userReactionTypeMap, err := c.buildCommentUserReactionTypeMap(ctx, recordList, userID)
	if err != nil {
		return nil, err
	}

	list := make([]*shopappv1.CommentItem, 0, len(recordList))
	for _, record := range recordList {
...
func (c *CommentInfoCase) buildCommentItem(
	...
	item.Status = shopcommonv1.CommentStatus(record.Status)
...
func (c *CommentInfoCase) buildContentSegments(content string, highlightWords []string) []*shopappv1.CommentTextSegment {
	// 评价正文为空时，直接返回空片段列表。
	if content == "" {
		return []*shopappv1.CommentTextSegment{}
	}
	// 未配置高亮词时，整段正文按普通文本返回。
	if len(highlightWords) == 0 {
		return []*shopappv1.CommentTextSegment{{Text: content}}
	}

	result := make([]*shopappv1.CommentTextSegment, 0)
	remain := content
	for len(remain) > 0 {
		nextIndex := -1
		nextWord := ""
		...
		if nextIndex < 0 {
			result = append(result, &shopappv1.CommentTextSegment{Text: remain})
			break
		}
		// 高亮词前存在普通正文时，先输出普通片段。
		if nextIndex > 0 {
			result = append(result, &shopappv1.CommentTextSegment{Text: remain[:nextIndex]})
		}
		result = append(result, &shopappv1.CommentTextSegment{
			Text:      nextWord,
			Highlight: true,
		})
