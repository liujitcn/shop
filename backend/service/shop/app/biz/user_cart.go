...
type UserCartCase struct {
	...
	*data.UserCartRepository
	...
	mapper          *mapper.CopierMapper[shopappv1.UserCart, models.UserCart]
}
...
func NewUserCartCase(
	...
	userCartMapper := mapper.NewCopierMapper[shopappv1.UserCart, models.UserCart]()
	return &UserCartCase{
		BaseCase:           baseCase,
...
func (c *UserCartCase) ListUserCart(ctx context.Context) (*shopappv1.ListUserCartResponse, error) {
	...
	storeMap := make(map[int64]*shopappv1.UserCartStore, len(tenantStoreMap))
	userCartStores := make([]*shopappv1.UserCartStore, 0, len(tenantStoreMap))
	for _, item := range all {
		...
		cart.RecommendContext = &shopappv1.RecommendContext{
		...
		if store == nil {
			...
			store = &shopappv1.UserCartStore{
				Store: &shopappv1.TenantStore{Id: item.TenantStoreID},
				Goods: make([]*shopappv1.UserCart, 0),
			}
	...
	return &shopappv1.ListUserCartResponse{
		UserCartStores: userCartStores,
	}, nil
}

// CreateUserCart 创建用户购物车
func (c *UserCartCase) CreateUserCart(ctx context.Context, userCart *shopappv1.CreateUserCartRequest) error {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return err
	}
...
func (c *UserCartCase) UpdateUserCart(ctx context.Context, req *shopappv1.UserCartForm) error {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return err
	}
...
func (c *UserCartCase) dispatchRecommendAddCartEvent(userID, goodsID, goodsNum int64, recommendContext *shopappv1.RecommendContext) {
	...
	queue.DispatchRecommendEvent(&dto.RecommendActor{
		...
		ActorID:   userID,
	}, &shopappv1.RecommendEventReportRequest{
		EventType: shopcommonv1.RecommendEventType(_const.RECOMMEND_EVENT_TYPE_ADD_CART),
