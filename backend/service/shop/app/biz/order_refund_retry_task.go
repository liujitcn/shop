...
var _ job.TaskExec = (*OrderRefundRetry)(nil)
...
func (t *OrderRefundRetry) Exec(args map[string]string) ([]string, error) {
	...
	opts = append(opts, repository.Where(query.RefundState.Eq(shopappv1.RefundResource_PROCESSING.String())))
	...
	for _, orderRefund := range orderRefunds {
		...
		if refundResource.GetRefundStatus() == shopappv1.RefundResource_PROCESSING {
			processingCount++
		} else {
			syncedCount++
		}
