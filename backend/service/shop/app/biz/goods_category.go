...
type GoodsCategoryCase struct {
	...
	*data.GoodsCategoryRepository
	...
	mapper        *mapper.CopierMapper[shopappv1.GoodsCategory, models.GoodsCategory]
	goodsMapper   *mapper.CopierMapper[shopappv1.GoodsInfo, models.GoodsInfo]
}
...
func NewGoodsCategoryCase(baseCase *biz.BaseCase, goodsCategoryRepo *data.GoodsCategoryRepository, goodsInfoRepo *data.GoodsInfoRepository) *GoodsCategoryCase {
	goodsMapper := mapper.NewCopierMapper[shopappv1.GoodsInfo, models.GoodsInfo]()
	...
	return &GoodsCategoryCase{
		...
		mapper:                  mapper.NewCopierMapper[shopappv1.GoodsCategory, models.GoodsCategory](),
		goodsMapper:             goodsMapper,
	}
}

// ListGoodsCategory 查询分类列表
func (c *GoodsCategoryCase) ListGoodsCategory(ctx context.Context, req *shopappv1.ListGoodsCategoryRequest) (*shopappv1.ListGoodsCategoryResponse, error) {
	...
	list := make([]*shopappv1.GoodsCategory, 0, len(all))
	...
	for _, item := range all {
		...
		if category.ParentId > 0 {
			...
			category.Goods = make([]*shopappv1.GoodsInfo, 0, len(goodsInfoList))
	...
	return &shopappv1.ListGoodsCategoryResponse{
