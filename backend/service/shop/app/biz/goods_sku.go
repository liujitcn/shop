...
type GoodsSKUCase struct {
	...
	*data.GoodsSKURepository
	mapper *mapper.CopierMapper[shopappv1.GoodsInfoResponse_Sku, models.GoodsSKU]
}
...
func NewGoodsSKUCase(baseCase *biz.BaseCase, goodsSKURepo *data.GoodsSKURepository) *GoodsSKUCase {
	return &GoodsSKUCase{
		...
		mapper: func() *mapper.CopierMapper[shopappv1.GoodsInfoResponse_Sku, models.GoodsSKU] {
			m := mapper.NewCopierMapper[shopappv1.GoodsInfoResponse_Sku, models.GoodsSKU]()
...
// 查询商品下的全部规格库存列表
func (c *GoodsSKUCase) listByGoodsID(ctx context.Context, goodsID int64, member bool) ([]*shopappv1.GoodsInfoResponse_Sku, error) {
	query := c.Query(ctx).GoodsSKU
	opts := make([]repository.QueryOption, 0, 1)
	...
	list := make([]*shopappv1.GoodsInfoResponse_Sku, 0)
	for _, item := range all {
		price := item.Price
		// 会员用户优先使用会员价展示 SKU。
		if member {
