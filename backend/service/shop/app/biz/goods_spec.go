...
type GoodsSpecCase struct {
	...
	*data.GoodsSpecRepository
	mapper *mapper.CopierMapper[shopappv1.GoodsInfoResponse_Spec, models.GoodsSpec]
}
...
func NewGoodsSpecCase(baseCase *biz.BaseCase, goodsSpecRepo *data.GoodsSpecRepository) *GoodsSpecCase {
	return &GoodsSpecCase{
		...
		mapper:              mapper.NewCopierMapper[shopappv1.GoodsInfoResponse_Spec, models.GoodsSpec](),
	}
}

// 查询商品下的全部规格列表
func (c *GoodsSpecCase) listByGoodsID(ctx context.Context, goodsID int64) ([]*shopappv1.GoodsInfoResponse_Spec, error) {
	...
	list := make([]*shopappv1.GoodsInfoResponse_Spec, 0)
	for _, item := range all {
		spec := c.mapper.ToDTO(item)
		items := _string.ConvertJsonStringToStringArray(item.Item)
		itemList := make([]*shopappv1.GoodsInfoResponse_Spec_Item, 0, len(items))
		...
		for _, name := range items {
			...
			itemList = append(itemList, &shopappv1.GoodsInfoResponse_Spec_Item{Name: name})
		}
		spec.Item = itemList
