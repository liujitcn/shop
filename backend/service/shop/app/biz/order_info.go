...
type OrderInfoCase struct {
	...
	*data.OrderInfoRepository
	...
	baseDictItemCase   *systemappbiz.BaseDictItemCase
	orderSchedulerCase *OrderSchedulerCase
	payCase            *PayCase
	wxPayCase          *wx.WxPayCase
	mapper             *mapper.CopierMapper[shopappv1.OrderInfo, models.OrderInfo]
}
...
func NewOrderInfoCase(
	...
	c := &OrderInfoCase{
		...
		payCase:             payCase,
		wxPayCase:           wxPayCase,
		mapper:              mapper.NewCopierMapper[shopappv1.OrderInfo, models.OrderInfo](),
	}
	...
	for _, item := range list {
		...
		if countdown < 0 {
			// 自动取消订单
			err = c.cancelOrder(context.Background(), item.UserID, &shopappv1.CancelOrderInfoRequest{
				TradeId: item.ID,
			})
...
func (c *OrderInfoCase) RefundOrderInfo(ctx context.Context, req *shopappv1.RefundOrderInfoRequest) error {
	...
	if orderInfo.Status != _const.ORDER_INFO_STATUS_WAIT_SHIPMENT {
		return errorsx.StateConflict(
			fmt.Sprintf("订单状态错误：【%s】", shopcommonv1.OrderInfoStatus_name[orderInfo.Status]),
			"order_info",
	...
	// 只有在线支付交易才会走微信退款流程。
	if shopcommonv1.OrderPayType(orderTrade.PayType) == shopcommonv1.OrderPayType(_const.ORDER_PAY_TYPE_ONLINE_PAY) {
		...
		// 仅微信支付订单需要调用微信退款接口
		if shopcommonv1.OrderPayChannel(orderTrade.PayChannel) == shopcommonv1.OrderPayChannel(_const.ORDER_PAY_CHANNEL_WX_PAY) {
				...
				if apiErr, ok := errors.AsType[*wxPayCore.APIError](err); ok {
					...
					if apiErr.Code == "INVALID_REQUEST" && apiErr.Message == "订单已全额退款" {
						...
						return errorsx.StateConflict(
							"订单已退款，不能重复退款",
							"order_refund",
							shopappv1.RefundResource_RefundStatus(_const.REFUND_RESOURCE_STATUS_SUCCESS).String(),
							"NONE",
						)
					}
				}
...
func (c *OrderInfoCase) recoverCompletedOrderRefund(ctx context.Context, orderTrade *models.OrderTrade, orderInfo *models.OrderInfo, currentRefund *models.OrderRefund) error {
	currentRefund.RefundState = shopappv1.RefundResource_ABNORMAL.String()
	...
	for _, orderRefund := range orderRefunds {
		...
		if queryErr != nil || refundResource.GetRefundStatus() != shopappv1.RefundResource_SUCCESS {
			continue
		}
		return c.payCase.RefundSuccess(ctx, orderTrade, refundResource)
	}
...
func (c *OrderInfoCase) ConfirmOrderInfo(ctx context.Context) (*shopappv1.ConfirmOrderInfoResponse, error) {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return nil, err
	}
	...
	createOrderGoods := make([]*shopappv1.CreateOrderInfoGoods, 0)
	for _, item := range userCartList {
		createOrderGoods = append(createOrderGoods, &shopappv1.CreateOrderInfoGoods{
			GoodsId: item.GoodsID,
			SkuCode: item.SKUCode,
			Num:     item.Num,
			RecommendContext: &shopappv1.RecommendContext{
...
func (c *OrderInfoCase) BuyNowOrderInfo(ctx context.Context, req *shopappv1.BuyNowOrderInfoRequest) (*shopappv1.BuyNowOrderInfoResponse, error) {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return nil, err
	}
...
func (c *OrderInfoCase) RepurchaseOrderInfo(ctx context.Context, req *shopappv1.RepurchaseOrderInfoRequest) (*shopappv1.RepurchaseOrderInfoResponse, error) {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return nil, err
	}
	...
	return &shopappv1.RepurchaseOrderInfoResponse{
		OrderGoodsStores: res.GetOrderGoodsStores(),
		Summary:          res.GetSummary(),
		ClearCart:        res.GetClearCart(),
	}, nil
...
func (c *OrderInfoCase) CountOrderInfo(ctx context.Context) (*shopappv1.CountOrderInfoResponse, error) {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return nil, err
	}
	var pendingPaymentCount int32
	pendingPaymentCount, err = c.countPageOrderInfo(ctx, authInfo.UserId, &shopappv1.PageOrderInfoRequest{
		TradeStatus: shopcommonv1.OrderTradeStatus_PENDING_PAYMENT_OTS.Enum(),
	})
	if err != nil {
		return nil, err
	}
	var waitShipmentCount int32
	waitShipmentCount, err = c.countPageOrderInfo(ctx, authInfo.UserId, &shopappv1.PageOrderInfoRequest{
		Status: shopcommonv1.OrderInfoStatus_WAIT_SHIPMENT_OIS.Enum(),
	})
	if err != nil {
		return nil, err
	}
	var shippedCount int32
	shippedCount, err = c.countPageOrderInfo(ctx, authInfo.UserId, &shopappv1.PageOrderInfoRequest{
		Status: shopcommonv1.OrderInfoStatus_SHIPPED_OIS.Enum(),
	})
	if err != nil {
		return nil, err
	}
	...
	refundableCount, err = c.countPageOrderInfo(ctx, authInfo.UserId, &shopappv1.PageOrderInfoRequest{
		Refundable: &refundable,
	})
	if err != nil {
		return nil, err
	}
	return &shopappv1.CountOrderInfoResponse{
		Counts: []*shopappv1.CountOrderInfoResponse_Count{
			{
				TradeStatus: shopcommonv1.OrderTradeStatus_PENDING_PAYMENT_OTS,
				Num:         pendingPaymentCount,
			},
			{
				Status: shopcommonv1.OrderInfoStatus_WAIT_SHIPMENT_OIS,
				Num:    waitShipmentCount,
			},
			{
				Status: shopcommonv1.OrderInfoStatus_SHIPPED_OIS,
				Num:    shippedCount,
			},
			{
				Refundable: true,
...
func (c *OrderInfoCase) countPageOrderInfo(ctx context.Context, userID int64, req *shopappv1.PageOrderInfoRequest) (int32, error) {
...
func (c *OrderInfoCase) PageOrderInfo(ctx context.Context, req *shopappv1.PageOrderInfoRequest) (*shopappv1.PageOrderInfoResponse, error) {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return nil, err
	}
	...
	var orderGoodsStoreMap map[int64][]*shopappv1.OrderGoodsStore
	...
	for _, trade := range trades {
		...
		var protoOrder *shopappv1.OrderInfo
	...
	return &shopappv1.PageOrderInfoResponse{
...
func (c *OrderInfoCase) pageOrderInfoQueryOptions(
	ctx context.Context,
	userID int64,
	req *shopappv1.PageOrderInfoRequest,
	...
	noStatusFilter := tradeStatus == shopcommonv1.OrderTradeStatus_UNKNOWN_OTS &&
		orderStatus == shopcommonv1.OrderInfoStatus_UNKNOWN_OIS &&
		refundStatus == shopcommonv1.OrderRefundStatus_UNKNOWN_ORS && req.HasRefund == nil && !refundableOnly
	aggregateTradeStatus := tradeStatus == shopcommonv1.OrderTradeStatus_PENDING_PAYMENT_OTS ||
		tradeStatus == shopcommonv1.OrderTradeStatus_PAYING_OTS ||
		tradeStatus == shopcommonv1.OrderTradeStatus_CLOSED_OTS

	var tradeOpts []repository.QueryOption
	// 待支付、支付中和已关闭按交易聚合展示，取消履约筛选同样映射到已关闭交易。
	if noStatusFilter || aggregateTradeStatus || orderStatus == shopcommonv1.OrderInfoStatus_CANCELED_OIS {
		query := c.orderTradeCase.Query(ctx).OrderTrade
		...
		if aggregateTradeStatus {
			...
			if tradeStatus == shopcommonv1.OrderTradeStatus_PENDING_PAYMENT_OTS {
		...
		} else if orderStatus == shopcommonv1.OrderInfoStatus_CANCELED_OIS {
	...
	childTradeStatus := tradeStatus != shopcommonv1.OrderTradeStatus_UNKNOWN_OTS && !aggregateTradeStatus
	queryChildOrders := noStatusFilter || childTradeStatus ||
		(orderStatus != shopcommonv1.OrderInfoStatus_UNKNOWN_OIS && orderStatus != shopcommonv1.OrderInfoStatus_CANCELED_OIS) ||
		refundStatus != shopcommonv1.OrderRefundStatus_UNKNOWN_ORS || req.HasRefund != nil || refundableOnly
	...
	if queryChildOrders {
		...
		if refundStatus != shopcommonv1.OrderRefundStatus_UNKNOWN_ORS {
...
func pageOrderItems(items []*appDto.OrderPageItem, pageNum, pageSize int64, prioritizeTrades bool) []*shopappv1.OrderInfo {
	...
	list := make([]*shopappv1.OrderInfo, 0, end-start)
...
func (c *OrderInfoCase) GetOrderInfoByID(ctx context.Context, id int64) (*shopappv1.OrderInfoResponse, error) {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return nil, err
	}
	...
	relatedOrders := make([]*shopappv1.OrderInfoResponse_RelatedOrder, 0, len(relatedOrderInfos))
	for _, relatedOrderInfo := range relatedOrderInfos {
		relatedOrders = append(relatedOrders, &shopappv1.OrderInfoResponse_RelatedOrder{
			OrderId: relatedOrderInfo.ID,
			OrderNo: relatedOrderInfo.OrderNo,
		})
	}
	// 查询订单收货地址快照
	var address *shopappv1.OrderInfoResponse_Address
	address, err = c.orderAddressCase.findByTradeID(ctx, orderInfo.TradeId)
	if err != nil {
		// 地址信息缺失时返回空地址，保持订单详情主体信息可展示。
		if errors.Is(err, gorm.ErrRecordNotFound) {
			address = &shopappv1.OrderInfoResponse_Address{}
		} else {
			return nil, err
		}
	}

	res := shopappv1.OrderInfoResponse{
		Order:         orderInfo,
		Address:       address,
	...
	if item.Status == _const.ORDER_INFO_STATUS_SHIPPED ||
		...
		var logistics *shopappv1.OrderInfoResponse_Logistics
		logistics, err = c.orderLogisticsCase.findByOrderID(ctx, orderInfo.Id)
		if err != nil {
			// 物流信息缺失时返回空物流，保持订单详情主体信息可展示。
			if errors.Is(err, gorm.ErrRecordNotFound) {
				logistics = &shopappv1.OrderInfoResponse_Logistics{}
			} else {
				return nil, err
			}
		}
...
func (c *OrderInfoCase) GetOrderTradeByID(ctx context.Context, tradeID int64) (*shopappv1.OrderInfoResponse, error) {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return nil, err
	}
	...
	var orderInfo *shopappv1.OrderInfo
...
func (c *OrderInfoCase) CreateOrderInfo(ctx context.Context, request *shopappv1.CreateOrderInfoRequest) (*shopappv1.CreateOrderInfoResponse, error) {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return nil, err
	}
	...
	if request.PayType == shopcommonv1.OrderPayType(_const.ORDER_PAY_TYPE_CASH_ON_DELIVERY) {
		tradeStatus = _const.ORDER_TRADE_STATUS_CASH_ON_DELIVERY
		orderStatus = _const.ORDER_INFO_STATUS_WAIT_SHIPMENT
	}

	storeOptions := make(map[int64]*shopappv1.OrderStoreOption, len(request.GetOrderStoreOptions()))
	...
	return &shopappv1.CreateOrderInfoResponse{
		TradeId:  orderTrade.ID,
		TradeNo:  orderTrade.TradeNo,
		OrderIds: orderIDs,
	}, nil
...
func (c *OrderInfoCase) DeleteOrderTrade(ctx context.Context, tradeID int64) error {
	...
	if orderTrade.Status != _const.ORDER_TRADE_STATUS_CLOSED {
		return errorsx.StateConflict(
			...
			shopcommonv1.OrderTradeStatus(_const.ORDER_TRADE_STATUS_CLOSED).String(),
		)
	}
...
func (c *OrderInfoCase) CancelOrderInfo(ctx context.Context, req *shopappv1.CancelOrderInfoRequest) error {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return err
	}
...
func (c *OrderInfoCase) ReceiveOrderInfo(ctx context.Context, req *shopappv1.ReceiveOrderInfoRequest) error {
	authInfo, err := c.GetAuthInfo(ctx)
	if err != nil {
		return err
	}
	...
	if orderInfo.Status != _const.ORDER_INFO_STATUS_SHIPPED {
		return errorsx.StateConflict(
			...
			"order_info",
			shopcommonv1.OrderInfoStatus(orderInfo.Status).String(),
			shopcommonv1.OrderInfoStatus(_const.ORDER_INFO_STATUS_SHIPPED).String(),
		)
	}
...
// 将订单模型转换为接口响应
func (c *OrderInfoCase) convertToProto(item *models.OrderInfo) *shopappv1.OrderInfo {
...
func (c *OrderInfoCase) applyTradeToProto(orderInfo *shopappv1.OrderInfo, orderTrade *models.OrderTrade) {
	orderInfo.TradeId = orderTrade.ID
...
func (c *OrderInfoCase) buildTradeOrderProto(
	...
	res := &shopappv1.OrderInfo{
		...
		PayType:          shopcommonv1.OrderPayType(orderTrade.PayType),
		PayChannel:       shopcommonv1.OrderPayChannel(orderTrade.PayChannel),
		TradeStatus:      shopcommonv1.OrderTradeStatus(orderTrade.Status),
		IsTrade:          true,
		Status:           shopcommonv1.OrderInfoStatus_NOT_STARTED_OIS,
	...
	if orderTrade.Status == _const.ORDER_TRADE_STATUS_CLOSED {
		res.Status = shopcommonv1.OrderInfoStatus_CANCELED_OIS
	}
...
// 汇总下单商品信息并生成确认单
func (c *OrderInfoCase) orderBuy(ctx context.Context, member bool, createOrderGoods []*shopappv1.CreateOrderInfoGoods) (*shopappv1.ConfirmOrderInfoResponse, error) {
	...
	var summary shopappv1.OrderSummary
	...
	// 当前版本统一免运费
	summary.PostFee = 0
	return &shopappv1.ConfirmOrderInfoResponse{
...
func (c *OrderInfoCase) buildOrderGoodsStoreMap(ctx context.Context, orderGoodsMap map[int64][]*models.OrderGoods) (map[int64][]*shopappv1.OrderGoodsStore, error) {
	res := make(map[int64][]*shopappv1.OrderGoodsStore, len(orderGoodsMap))
	storeIDs := make([]int64, 0)
	storeIDSet := make(map[int64]bool)
	for orderID, orderGoodsList := range orderGoodsMap {
		orderStoreMap := make(map[int64]*shopappv1.OrderGoodsStore)
		orderGoodsStores := make([]*shopappv1.OrderGoodsStore, 0)
		for _, orderGoods := range orderGoodsList {
			orderStore := orderStoreMap[orderGoods.TenantStoreID]
			// 门店首次命中时，初始化对应的订单商品分组。
			if orderStore == nil {
				orderStore = &shopappv1.OrderGoodsStore{
					Store:   &shopappv1.TenantStore{Id: orderGoods.TenantStoreID},
					Goods:   make([]*shopappv1.OrderGoods, 0),
					Summary: &shopappv1.OrderSummary{},
				}
...
func applyOrderStoreOptions(orderGoodsStores []*shopappv1.OrderGoodsStore, orderInfos []*models.OrderInfo) {
	...
	for _, orderGoodsStore := range orderGoodsStores {
		...
		orderGoodsStore.DeliveryTime = shopcommonv1.OrderDeliveryTime(orderInfo.DeliveryTime)
		orderGoodsStore.Remark = orderInfo.Remark
	}
}

// dispatchRecommendOrderEvent 根据已落库订单事实回写推荐下单事件。
func (c *OrderInfoCase) dispatchRecommendOrderEvent(payType shopcommonv1.OrderPayType, userID int64, goodsList []*shopappv1.CreateOrderInfoGoods, eventTime time.Time) {
	// 主体编号非法或订单商品为空时，无法构建可归因的推荐下单事件。
	if userID <= 0 || len(goodsList) == 0 {
		return
	}

	eventTypeList := make([]shopcommonv1.RecommendEventType, 0, 2)
	eventTypeList = append(eventTypeList, shopcommonv1.RecommendEventType(_const.RECOMMEND_EVENT_TYPE_ORDER_CREATE))
	// 货到付款订单在创建时同步回写支付行为。
	if payType == shopcommonv1.OrderPayType(_const.ORDER_PAY_TYPE_CASH_ON_DELIVERY) {
		eventTypeList = append(eventTypeList, shopcommonv1.RecommendEventType(_const.RECOMMEND_EVENT_TYPE_ORDER_PAY))
	}
	...
	for _, eventType := range eventTypeList {
		for _, goodsItem := range goodsList {
			...
			orderEventReport := &shopappv1.RecommendEventReportRequest{
				EventType: eventType,
...
func (c *OrderInfoCase) scheduleTradeCancellation(tradeID, userID int64, delay time.Duration) {
	...
	c.orderSchedulerCase.AddSchedule(tradeID, delay, func() {
		err := c.cancelOrder(context.Background(), userID, &shopappv1.CancelOrderInfoRequest{TradeId: tradeID})
		if err == nil {
			return
		}
...
func (c *OrderInfoCase) cancelOrder(ctx context.Context, userID int64, req *shopappv1.CancelOrderInfoRequest) error {
	...
	if shopcommonv1.OrderPayType(orderTrade.PayType) == shopcommonv1.OrderPayType(_const.ORDER_PAY_TYPE_ONLINE_PAY) &&
		shopcommonv1.OrderPayChannel(orderTrade.PayChannel) == shopcommonv1.OrderPayChannel(_const.ORDER_PAY_CHANNEL_WX_PAY) {
		...
		if paymentResource != nil {
			...
			if paymentResource.GetTradeState() == shopappv1.PaymentResource_SUCCESS ||
				paymentResource.GetTradeState() == shopappv1.PaymentResource_REFUND {
				...
				return errorsx.StateConflict(
					...
					shopappv1.PaymentResource_NOTPAY.String(),
				)
			}
			// 尚未关闭的微信支付单必须先关单，关单成功后才允许回退本地库存。
			if paymentResource.GetTradeState() != shopappv1.PaymentResource_CLOSED &&
				paymentResource.GetTradeState() != shopappv1.PaymentResource_REVOKED {
				closeErr := c.wxPayCase.CloseOrderByOutTradeNo(orderTrade.TradeNo)
				if closeErr != nil {
					// 关单与支付并发时重新查询，若已支付则同步本地状态并拒绝取消。
					paymentResource, err = c.wxPayCase.QueryOrderByOutTradeNo(orderTrade.TradeNo)
					if err == nil && paymentResource.GetTradeState() == shopappv1.PaymentResource_SUCCESS {
						err = c.payCase.PaySuccess(ctx, orderTrade, paymentResource)
						if err != nil {
							return err
						}
...
func (c *OrderInfoCase) markTradeClosed(ctx context.Context, orderTrade *models.OrderTrade) error {
	...
	return errorsx.StateConflict(
		fmt.Sprintf("交易状态错误：【%s】", shopcommonv1.OrderTradeStatus_name[currentTrade.Status]),
		"order_trade",
		shopcommonv1.OrderTradeStatus(currentTrade.Status).String(),
		"PENDING_PAYMENT_OTS|PAYING_OTS",
	)
}
