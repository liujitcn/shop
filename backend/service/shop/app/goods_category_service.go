// Code generated by protoc-gen-go-service.
// versions:
// - protoc-gen-go-service v0.0.1
// - protoc             (unknown)
// source: app/goods_category.proto

package app

import (
	"context"
	"fmt"

	shopappv1 "shop/api/gen/go/shop/app/v1"
	"shop/pkg/errorsx"
	"shop/service/shop/app/biz"

	"github.com/go-kratos/kratos/v3/log"
	"google.golang.org/grpc"
)

const _ = grpc.SupportPackageIsVersion7

// GoodsCategoryService 商品分类服务
type GoodsCategoryService struct {
	shopappv1.UnimplementedGoodsCategoryServiceServer
	goodsCategoryCase *biz.GoodsCategoryCase
}

// NewGoodsCategoryService 创建商品分类服务
func NewGoodsCategoryService(
	goodsCategoryCase *biz.GoodsCategoryCase,
) *GoodsCategoryService {
	var ss = GoodsCategoryService{
		goodsCategoryCase: goodsCategoryCase,
	}
	return &ss
}

// ListGoodsCategory 查询商品分类列表
func (s *GoodsCategoryService) ListGoodsCategory(ctx context.Context, req *shopappv1.ListGoodsCategoryRequest) (*shopappv1.ListGoodsCategoryResponse, error) {
	res, err := s.goodsCategoryCase.ListGoodsCategory(ctx, req)
	if err != nil {
		log.Error(fmt.Sprintf("ListGoodsCategory %v", err))
		return nil, errorsx.WrapInternal(err, "查询商品分类列表失败")
	}
	return res, nil
}
