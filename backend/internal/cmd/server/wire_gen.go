...
func initApp(context *bootstrap.Context) (*kratos.App, func(), error) {
	...
	recommendEventRepo := data.NewRecommendEventRepo(dataData)
	...
	baseJobCase := biz2.NewBaseJobCase(baseCase, baseJobRepo, baseJobLogCase, cronServer)
	...
	recommendEventCase := biz2.NewRecommendEventCase(baseCase, recommendEventRepo)
	recommendRequestItemCase := biz2.NewRecommendRequestItemCase(baseCase, recommendRequestItemRepo, goodsInfoRepo, recommendEventCase)
	recommendRequestCase := biz2.NewRecommendRequestCase(baseCase, recommendRequestRepo, baseUserRepo, recommendRequestItemCase, recommendEventCase)
	recommendRequestService := admin.NewRecommendRequestService(recommendRequestCase)
	recommendRemoteCase := biz2.NewRecommendRemoteCase(remoteRecommend)
	recommendRemoteService := admin.NewRecommendRemoteService(recommendRemoteCase)
	...
	payCase := biz4.NewPayCase(baseCase, transaction, orderInfoRepo, orderGoodsRepo, orderPaymentRepo, orderRefundRepo, orderSchedulerCase, wxPayCase)
	...
	bizRecommendRequestCase := biz4.NewRecommendRequestCase(baseCase, transaction, recommendRequestRepo, recommendRequestItemRepo)
	bizRecommendEventCase := biz4.NewRecommendEventCase(baseCase, recommendEventRepo, bizRecommendRequestCase)
	userCollectCase := biz4.NewUserCollectCase(baseCase, userCollectRepo, bizGoodsInfoCase, bizGoodsSkuCase)
	userReceiver := remote.NewUserReceiver(remoteRecommend)
	sessionReceiver := remote.NewSessionReceiver(remoteRecommend)
	namedReceiver := remote.NewNamedReceiver(remoteRecommend)
	chainReceiver := remote.NewChainReceiver(remoteRecommend, userReceiver, sessionReceiver, namedReceiver)
