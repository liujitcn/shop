...
`gorse` 目录用于本地启动 Gorse 推荐服务，方便后端推荐链路、管理后台 Gorse 推荐页面、推荐调试、同步与编排联调。Gorse 是推荐增强能力，不是商城端唯一的推荐来源；后端在远端不可用或候选不足时会继续走本地推荐责任链。
...
`config.toml` 同时定义反馈分类、非个性化热榜、协同过滤、相似内容、排序器和回退推荐器。请在配置文件中维护这些算法参数，不要在前端页面写死推荐器名称或把本地开发凭据复制到文档、客户端代码或生产配置。
...
- 管理后台通过后端代理访问 Gorse 推荐概览、任务、用户、商品、相似内容、反馈、高级调试、推荐编排和推荐配置能力；浏览器不会直接持有 Gorse API Key。
...
| [Gorse 推荐服务管理平台功能设计](../frontend/admin/docs/gorse-recommend-design.md) | 已实现的后台能力和尚未后台化的演进边界。 |
