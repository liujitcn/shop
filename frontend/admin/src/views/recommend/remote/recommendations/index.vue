<template>
  <div class="remote-page remote-recommendations-page">
    <el-card class="remote-hero-card" shadow="never">
      <div class="remote-hero-card__content">
        <p>Gorse API</p>
        <h2>推荐调试</h2>
        <span>查询用户推荐、最新、热门、协同过滤和命名推荐器结果，便于核对远程推荐召回链路。</span>
      </div>
      <div class="remote-hero-card__actions">
        <el-button :loading="loading" @click="loadRecommendations">刷新</el-button>
      </div>
    </el-card>

    <el-card class="remote-toolbar-card" shadow="never">
      <el-form :inline="true" :model="query" @submit.prevent>
        <el-form-item label="推荐类型">
          <el-select v-model="query.type" filterable placeholder="请选择推荐类型" @change="handleTypeChange">
            <el-option v-for="item in recommendTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="requiresId" :label="idLabel">
          <el-input v-model.trim="query.id" clearable :placeholder="idPlaceholder" @keyup.enter="loadRecommendations" />
        </el-form-item>
        <el-form-item v-if="supportsCategory" label="分类">
          <el-select v-model="query.category" clearable filterable placeholder="全部分类">
            <el-option v-for="item in categoryOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="数量">
          <el-input-number v-model="query.n" :min="1" :max="200" :step="10" controls-position="right" />
        </el-form-item>
        <el-form-item label="偏移">
          <el-input-number v-model="query.offset" :min="0" :max="10000" :step="10" controls-position="right" />
        </el-form-item>
        <el-form-item label="写回类型">
          <el-input v-model.trim="query.writeBackType" clearable placeholder="如 read，可选" />
        </el-form-item>
        <el-form-item label="候选集">
          <el-input v-model.trim="query.candidates" clearable placeholder="商品编号逗号分隔，可选" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="loading" @click="loadRecommendations">查询</el-button>
          <el-button @click="resetQuery">重置</el-button>
        </el-form-item>
      </el-form>
    </el-card>

    <el-card class="remote-section-card" shadow="never">
      <template #header>
        <div class="remote-section-card__header">
          <strong>Recommendations</strong>
          <span>当前 {{ list.length }} 条结果</span>
        </div>
      </template>
      <el-table v-loading="loading" :data="list" :row-key="getRowKey" border>
        <el-table-column label="序号" type="index" width="80" />
        <el-table-column label="编号" min-width="180">
          <template #default="{ row }">{{ getItemId(row) || formatRemoteCell(row) }}</template>
        </el-table-column>
        <el-table-column label="分数" min-width="120" align="right">
          <template #default="{ row }">{{ formatRemoteCell(resolveRemoteValue(row, scoreKeys)) }}</template>
        </el-table-column>
        <el-table-column label="分类" min-width="180">
          <template #default="{ row }">
            <div v-if="getCategories(row).length" class="remote-tag-list">
              <el-tag v-for="category in getCategories(row)" :key="String(category)" effect="plain" type="info">
                {{ formatRemoteCell(category) }}
              </el-tag>
            </div>
            <span v-else>--</span>
          </template>
        </el-table-column>
        <el-table-column label="标签/描述" min-width="320">
          <template #default="{ row }">
            <span class="remote-mono-text">{{ foldRemoteValue(resolveRemoteValue(row, labelKeys) ?? row) }}</span>
          </template>
        </el-table-column>
      </el-table>
    </el-card>

    <JsonPanel
      title="原始响应"
      description="远程推荐接口返回的原始 JSON。"
      :json="rawJson"
      :loading="loading"
      @refresh="loadRecommendations"
    />
  </div>
</template>

<script setup lang="ts">
import { computed, onMounted, reactive, ref } from "vue";
import { useRoute } from "vue-router";
import { ElMessage } from "element-plus";
import JsonPanel from "../components/JsonPanel.vue";
import { defRecommendRemoteService } from "@/api/admin/recommend_remote";
import {
  foldRemoteValue,
  formatRemoteCell,
  formatRemoteObject,
  buildCategoryRows,
  resolveRemoteArray,
  resolveRemoteId,
  resolveRemoteValue,
  type RemoteRecord
} from "../utils";

defineOptions({ name: "RemoteRecommendations" });

interface SelectOption {
  label: string;
  value: string;
}

const itemIdKeys = ["ItemId", "itemId", "item_id", "Id", "id"];
const scoreKeys = ["Score", "score", "Value", "value"];
const labelKeys = ["Labels", "labels", "Comment", "comment", "Description", "description"];
const categoryKeys = ["Categories", "categories", "Category", "category"];
const recommendTypeOptions = ref<SelectOption[]>([
  { label: "用户推荐", value: "recommend" },
  { label: "最新推荐", value: "latest" },
  { label: "热门推荐", value: "popular" },
  { label: "协同过滤", value: "collaborative" }
]);
const categoryOptions = ref<SelectOption[]>([{ label: "全部分类", value: "" }]);
const query = reactive({
  type: "recommend",
  id: "",
  category: "",
  n: 20,
  offset: 0,
  writeBackType: "",
  candidates: ""
});
const route = useRoute();
const loading = ref(false);
const list = ref<RemoteRecord[]>([]);
const rawJson = ref("[]");

const requiresId = computed(() => !["latest", "popular"].includes(query.type));
const supportsCategory = computed(() => !query.type.startsWith("user-to-user/"));
const idLabel = computed(() => (query.type.startsWith("item-to-item/") ? "商品编号" : "用户编号"));
const idPlaceholder = computed(() => (query.type.startsWith("item-to-item/") ? "输入商品编号" : "输入用户编号"));

/** 加载远程推荐配置和分类，用于构建查询条件。 */
async function loadOptions() {
  await Promise.allSettled([loadConfigOptions(), loadCategoryOptions()]);
}

/** 从远程配置中提取命名推荐器。 */
async function loadConfigOptions() {
  const data = await defRecommendRemoteService.GetConfig({});
  const config = data.config ?? {};
  const recommend = config.recommend ?? config.Recommend ?? {};
  const itemToItem = Array.isArray(recommend["item-to-item"]) ? recommend["item-to-item"] : [];
  const userToUser = Array.isArray(recommend["user-to-user"]) ? recommend["user-to-user"] : [];
  const namedOptions: SelectOption[] = [];
  itemToItem.forEach((item: RemoteRecord) => {
    const name = String(resolveRemoteValue(item, ["name", "Name"]) ?? "");
    if (name) namedOptions.push({ label: `Item to Item / ${name}`, value: `item-to-item/${name}` });
  });
  userToUser.forEach((item: RemoteRecord) => {
    const name = String(resolveRemoteValue(item, ["name", "Name"]) ?? "");
    if (name) namedOptions.push({ label: `User to User / ${name}`, value: `user-to-user/${name}` });
  });
  recommendTypeOptions.value = recommendTypeOptions.value.concat(namedOptions);
}

/** 加载远程分类选项。 */
async function loadCategoryOptions() {
  const data = await defRecommendRemoteService.GetCategory({});
  categoryOptions.value = [{ label: "全部分类", value: "" }].concat(
    buildCategoryRows(data.list).map(item => ({ label: item.name, value: item.name }))
  );
}

/** 查询远程推荐结果。 */
async function loadRecommendations() {
  if (requiresId.value && !query.id) {
    ElMessage.warning(`请先输入${idLabel.value}`);
    return;
  }
  loading.value = true;
  try {
    const data = await defRecommendRemoteService.GetRecommendation({ ...query });
    rawJson.value = formatRemoteObject(data.list);
    list.value = data.list.map(item => (item.raw ?? item) as RemoteRecord);
  } catch (error) {
    ElMessage.error("查询远程推荐结果失败");
    throw error;
  } finally {
    loading.value = false;
  }
}

/** 推荐类型变化时清理不适用条件。 */
function handleTypeChange() {
  if (!supportsCategory.value) query.category = "";
}

/** 重置查询条件。 */
function resetQuery() {
  query.type = "recommend";
  query.id = "";
  query.category = "";
  query.n = 20;
  query.offset = 0;
  query.writeBackType = "";
  query.candidates = "";
}

/** 获取结果行键。 */
function getRowKey(row: RemoteRecord, index: number) {
  return `${getItemId(row) || index}-${index}`;
}

/** 获取商品编号。 */
function getItemId(row: RemoteRecord) {
  return resolveRemoteId(row, itemIdKeys);
}

/** 获取分类集合。 */
function getCategories(row: RemoteRecord) {
  return resolveRemoteArray(row, categoryKeys);
}

onMounted(async () => {
  applyRouteQuery();
  await loadOptions();
});

/** 应用其他页面跳转携带的查询参数。 */
function applyRouteQuery() {
  const type = route.query.type;
  const id = route.query.id;
  if (typeof type === "string" && type) query.type = type;
  if (typeof id === "string" && id) query.id = id;
}
</script>

<style scoped lang="scss">
.remote-page {
  display: flex;
  flex-direction: column;
  gap: 16px;
}

.remote-hero-card,
.remote-toolbar-card,
.remote-section-card {
  border-color: var(--admin-page-card-border);
  background: var(--admin-page-card-bg);
  box-shadow: var(--admin-page-shadow);
}

.remote-hero-card {
  background: radial-gradient(circle at top right, var(--el-color-primary-light-9), transparent 38%), var(--admin-page-card-bg);

  :deep(.el-card__body) {
    display: flex;
    align-items: center;
    justify-content: space-between;
    gap: 16px;
  }
}

.remote-hero-card__content {
  display: flex;
  flex-direction: column;
  gap: 8px;

  p {
    margin: 0;
    color: var(--admin-page-text-secondary);
  }

  h2 {
    margin: 0;
    color: var(--admin-page-text-primary);
  }

  span {
    color: var(--admin-page-text-secondary);
  }
}

.remote-section-card__header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
  color: var(--admin-page-text-primary);

  span {
    color: var(--admin-page-text-secondary);
    font-size: 13px;
  }
}

.remote-tag-list {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
}

.remote-mono-text {
  font-family: var(--admin-page-font-mono);
  word-break: break-all;
}
</style>
