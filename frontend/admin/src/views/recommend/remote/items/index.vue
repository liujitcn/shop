  ...
  <div class="remote-page">
    <el-card class="remote-page__toolbar" shadow="never">
      <el-form :inline="true" :model="query" @submit.prevent>
        <el-form-item label="商品编号">
          <el-input v-model.trim="query.id" clearable placeholder="输入编号可查询单个商品" @keyup.enter="loadItems(true)" />
        </el-form-item>
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="list" :row-key="getItemId" border>
        <el-table-column prop="ItemId" label="商品编号" min-width="180">
          <template #default="{ row }">{{ getItemId(row) || "--" }}</template>
        </el-table-column>
        <el-table-column prop="Categories" label="分类" min-width="220">
          <template #default="{ row }">{{ formatRemoteCell(row.Categories ?? row.categories) }}</template>
        </el-table-column>
        <el-table-column prop="IsHidden" label="隐藏" min-width="100">
          <template #default="{ row }">{{ formatRemoteCell(row.IsHidden ?? row.isHidden ?? row.is_hidden) }}</template>
        </el-table-column>
    ...
    <JsonPanel
      title="原始响应"
      description="远程推荐商品接口原始 JSON。"
      :json="rawJson"
      :loading="loading"
      @refresh="loadItems(false)"
    />
...
async function loadItems(resetCursor = false) {
  ...
  loading.value = true;
  try {
    // 输入商品编号时直接查询单个商品，避免远程列表接口不支持模糊筛选。
    if (query.id) {
...
async function deleteItem(row: RemoteRecord) {
  ...
  await ElMessageBox.confirm(`是否确定删除远程推荐商品？\n商品编号：${id}`, "警告", {
