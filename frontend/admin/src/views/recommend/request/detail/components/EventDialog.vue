      ...
      <div class="recommend-request-event-dialog__summary">
        <el-descriptions :column="2" border class="recommend-request-event-dialog__descriptions">
          ...
          <el-descriptions-item label="商品状态">
            <DictLabel :model-value="currentItem?.goodsStatus" code="goods_status" />
...
import type { GetRecommendRequestEventResponse, RecommendRequestItem } from "@/rpc/admin/recommend_request";
...
function openDialog(options: RecommendRequestEventDialogOpenOptions) {
  if (!options.requestRecordId || !options.item?.goodsId) {
...
async function queryEventData() {
  ...
  if (!currentRequestRecordId.value || !item?.goodsId) {
    resetEventData();
    return;
  }
  ...
  try {
    const data = await defRecommendRequestService.GetRecommendRequestEvent({
      requestRecordId: currentRequestRecordId.value,
