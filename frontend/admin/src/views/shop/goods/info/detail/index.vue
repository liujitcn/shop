...
import { type GoodsInfoForm } from "@/rpc/shop/admin/v1/goods_info";
import { type GoodsSku } from "@/rpc/shop/admin/v1/goods_sku";
import { defGoodsInfoService } from "@/api/shop/goods_info";
import { defCommentInfoService } from "@/api/shop/comment_info";
...
import { CommentStatus, GoodsStatus } from "@/rpc/shop/common/v1/enum";
...
import { buildTenantStoreDisplayMap, DEFAULT_TENANT_CODE, type TenantStoreDisplayInfo } from "@/views/shop/utils/tenant";
...
function isCurrentGoodsDetailRoute() {
  return route.name === "GoodsInfoDetail" || route.path.includes("/goods/info/detail/");
}
...
function handleOpenPendingDiscussion(row: CommentInfo) {
  ...
  void navigateTo(router, `/comment/info/detail/${row.id}`, {
    tab: "discussion",
...
.goods-cover-panel {
  ...
  display: flex;
  min-width: 0;
  height: calc(var(--goods-summary-toolbar-height) + var(--goods-summary-panel-gap) + var(--goods-summary-card-height) * 2 + var(--goods-summary-card-row-gap));
  padding: 8px;
...
.goods-summary-grid {
  ...
  gap: var(--goods-summary-card-row-gap) var(--goods-summary-card-column-gap);
