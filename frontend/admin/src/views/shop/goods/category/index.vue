...
import type { GoodsCategory, GoodsCategoryForm } from "@/rpc/shop/admin/v1/goods_category";
    ...
    actions: [
      {
        ...
        link: true,
        icon: CirclePlus,
        hidden: scope => !BUTTONS.value["goods:category:create"] || Boolean((scope.row as GoodsCategory).parent_id),
      ...
      },
      {
        ...
        onClick: (_, params) =>
          handleOpenDialog(params?.parent_id as number | undefined, params?.categoryId as number | undefined)
      },
      {
        label: "删除",
        type: "danger",
...
async function handleOpenDialog(parent_id?: number, categoryId?: number) {
  ...
  if (categoryId) {
    defGoodsCategoryService.GetGoodsCategory({ id: categoryId }).then(data => {
...
function handleSubmit() {
  formDialogRef.value?.validate()?.then(valid => {
    ...
    const request = submitData.id
      ? defGoodsCategoryService.UpdateGoodsCategory({ id: submitData.id, goods_category: submitData })
      : defGoodsCategoryService.CreateGoodsCategory({ goods_category: submitData });
    request.then(() => {
...
function handleDelete(selected?: number | string | Array<number | string> | GoodsCategory | GoodsCategory[]) {
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    () => {
      defGoodsCategoryService.DeleteGoodsCategory({ ids: categoryIds }).then(() => {
