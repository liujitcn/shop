...
async function loadUserPage(cursor = currentCursor.value) {
  loading.value = true;
  try {
    const data = await defRecommendGorseService.PageUser({ cursor, n: pageSize.value });
...
function handleOpenSimilar(row: UserResponse) {
  ...
  navigateTo(router, `/gorse/user/similar/${row.user_id}`);
}
...
function handleOpenRecommend(row: UserResponse) {
  ...
  navigateTo(router, `/gorse/user/recommend/${row.user_id}`);
}
...
function handleOpenFeedback(row: UserResponse) {
  ...
  navigateTo(router, `/gorse/user/feedback/${row.user_id}`);
}
