...
import { defGoodsPropService } from "@/api/shop/admin/goods_prop";
import type { GoodsProp, PageGoodsPropRequest } from "@/rpc/shop/admin/v1/goods_prop";
...
function handleSubmit() {
  formDialogRef.value?.validate()?.then(isValid => {
    ...
    const request = submitData.id
      ? defGoodsPropService.UpdateGoodsProp({ id: submitData.id, goods_prop: submitData })
      : defGoodsPropService.CreateGoodsProp({ goods_prop: submitData });
    request.then(() => {
...
function handleDelete(selected?: number | string | Array<number | string> | GoodsProp | GoodsProp[]) {
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    () => {
      defGoodsPropService.DeleteGoodsProp({ ids: propIds }).then(() => {
