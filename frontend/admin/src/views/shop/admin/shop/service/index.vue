...
import { defShopServiceService } from "@/api/shop/admin/shop_service";
import type { PageShopServiceRequest, ShopService, ShopServiceForm } from "@/rpc/shop/admin/v1/shop_service";
...
function handleOpenDialog(serviceId?: number) {
  ...
  defShopServiceService.GetShopService({ id: serviceId }).then(data => {
...
function handleSubmit() {
  formDialogRef.value?.validate()?.then(valid => {
    ...
    const request = submitData.id
      ? defShopServiceService.UpdateShopService({ id: submitData.id, shop_service: submitData })
      : defShopServiceService.CreateShopService({ shop_service: submitData });
...
function handleDelete(selected?: number | string | Array<number | string> | ShopService | ShopService[]) {
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    () => {
      defShopServiceService.DeleteShopService({ ids: serviceIds }).then(() => {
