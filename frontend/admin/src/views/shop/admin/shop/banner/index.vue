...
import { defShopBannerService } from "@/api/shop/admin/shop_banner";
import type { PageShopBannerRequest, ShopBanner, ShopBannerForm } from "@/rpc/shop/admin/v1/shop_banner";
...
async function handleOpenDialog(bannerId?: number) {
  ...
  if (bannerId) {
    dialog.title = "修改轮播图";
    defShopBannerService.GetShopBanner({ id: bannerId }).then(data => {
...
function handleSubmit() {
  formDialogRef.value?.validate()?.then(valid => {
    ...
    const request = formData.id
      ? defShopBannerService.UpdateShopBanner({ id: formData.id, shop_banner: formData })
      : defShopBannerService.CreateShopBanner({ shop_banner: formData });
    request.then(() => {
...
function handleDelete(selected?: number | string | Array<number | string> | ShopBanner | ShopBanner[]) {
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    () => {
      defShopBannerService.DeleteShopBanner({ ids: bannerIds }).then(() => {
