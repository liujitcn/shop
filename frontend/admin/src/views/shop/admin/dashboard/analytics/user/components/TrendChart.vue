...
import { defUserAnalyticsService } from "@/api/shop/admin/user_analytics";
...
const option = computed<ECOption>(() => ({
  ...
  yAxis: [
    {
      type: "value",
      name: trendData.y_axis_names[0] || "人数",
      axisLabel: {
        color: "#6d7b8f"
      },
      splitLine: {
...
async function loadData(timeType: AnalyticsTimeType) {
  const data = await defUserAnalyticsService.TrendUserAnalytics({ time_type: timeType });
