    ...
    <el-card v-if="detailData.request" class="detail-hero-card" shadow="never">
      <div class="detail-summary-grid">
        ...
        <button type="button" class="detail-summary-card detail-summary-card--action" @click="handleSwitchTab('item')">
          ...
          <strong class="detail-summary-card__value">{{ detailData.item_list.length }}</strong>
        </button>
        <div class="detail-summary-card">
            ...
            <div class="detail-section-panel">
              ...
              <el-descriptions :column="2" border class="detail-descriptions">
                <el-descriptions-item label="主体类型">
                  <DictLabel :model-value="detailData.request?.actor_type" code="recommend_actor_type" />
                ...
                <el-descriptions-item label="请求时间">{{ detailData.request?.request_at || "--" }}</el-descriptions-item>
                <el-descriptions-item label="页码">{{ detailData.request?.page_num ?? "--" }}</el-descriptions-item>
                <el-descriptions-item label="分页大小">{{ detailData.request?.page_size ?? "--" }}</el-descriptions-item>
            ...
            <div class="detail-grid">
              <div class="detail-section-panel">
                ...
                <div v-if="detailTraceList.length" class="trace-flow">
                  <template v-for="(item, index) in detailTraceList" :key="`${item.provider_name}-${index}`">
                    <div
                      class="trace-flow__item"
                      :class="{
                        'trace-flow__item--hit': item.hit,
                        'trace-flow__item--final': item.is_final,
                        'trace-flow__item--error': Boolean(item.error_msg),
                    ...
                    >
                      <div class="trace-flow__top">
                        ...
                        <div class="trace-flow__tag">
                          <el-tag v-if="item.is_final" type="success" effect="light">最终命中</el-tag>
              ...
              <div class="detail-section-panel">
                ...
                <el-descriptions :column="1" border class="detail-descriptions">
                  <el-descriptions-item label="上下文推荐器">
                    <RecommendProviderLabel :strategy="strategyValue" :provider-name="detailData.context?.provider_name" />
                  </el-descriptions-item>
                </el-descriptions>
              ...
              >
                ...
                <template #event_count="scope">
                  <div class="detail-event-count-cell">
                    ...
                    <span v-else>{{ scope.row.event_count ?? 0 }}</span>
                  </div>
                </template>
              </ProTable>
            </div>
...
import RecommendProviderLabel from "@/views/shop/admin/recommend/request/components/ProviderLabel.vue";
import RecommendRequestEventDialog from "@/views/shop/admin/recommend/request/detail/components/EventDialog.vue";
import { defRecommendRequestService } from "@/api/shop/admin/recommend_request";
import type {
...
/** 当前请求展示的最终推荐器。 */
const finalProviderName = computed(() => detailData.context?.final_provider_name || detailData.request?.provider_name || "--");
...
/** 当前请求上下文 JSON 的格式化内容。 */
const formattedContextJson = computed(() => formatJson(detailData.context?.raw_json || "{}"));

/** 当前请求商品关联事件总数。 */
const totalEventCount = computed(() => detailData.item_list.reduce((sum, item) => sum + Number(item.event_count || 0), 0));
...
const itemColumns: ColumnProps[] = [
  ...
  },
  {
    ...
    minWidth: 130,
    align: "right",
    render: scope => formatPrice((scope.row as RecommendRequestItem).discount_price)
  },
  { prop: "goods_status", label: "商品状态", minWidth: 120, dictCode: "goods_status" },
  { prop: "event_count", label: "事件条数", minWidth: 160, showOverflowTooltip: false }
];
...
function handleQuery(targetRequestRecordId: number = requestRecordId.value) {
  ...
  defRecommendRequestService
    .GetRecommendRequest({ id: targetRequestRecordId })
    .then(data => {
...
function handleOpenGoodsDetail(item: RecommendRequestItem) {
  ...
  void navigateTo(router, `/goods/detail/${item.goods_id}`);
}
...
function handleOpenEventDialog(item: RecommendRequestItem) {
  if (Number(item.event_count || 0) <= 0) {
