...
import { defShopHotService } from "@/api/shop/admin/shop_hot";
import type { PageShopHotRequest, ShopHot, ShopHotForm } from "@/rpc/shop/admin/v1/shop_hot";
...
function handleOpenDialog(hotId?: number) {
  ...
  defShopHotService.GetShopHot({ id: hotId }).then(data => {
...
function handleSubmit() {
  formDialogRef.value?.validate()?.then(isValid => {
    ...
    const request = submitData.id
      ? defShopHotService.UpdateShopHot({ id: submitData.id, shop_hot: submitData })
      : defShopHotService.CreateShopHot({ shop_hot: submitData });
    request.then(() => {
...
function handleDelete(selected?: number | string | Array<number | string> | ShopHot | ShopHot[]) {
  ...
  ElMessageBox.confirm(confirmMessage, "警告", {
    ...
    () => {
      defShopHotService.DeleteShopHot({ ids: hotIds }).then(() => {
