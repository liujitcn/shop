...
import { defWorkspaceService, subscribeWorkspaceSseRefresh } from "@/api/shop/workspace";
...
import { DEFAULT_TENANT_CODE, parseTenantStoreTreeValue, transformTenantStoreTreeOptions } from "@/views/shop/utils/tenant";
...
const pendingCommentListPath = computed<RouteLocationRaw>(() => {
  return { path: "/comment/info", query: { status: String(CommentStatus.PENDING_REVIEW_CS) } };
});
  ...
  return [
    {
      ...
      analysisPath: { path: "/analytics/order", query: workspaceRouteQuery.value },
      actionPath: { path: "/analytics/order", query: workspaceRouteQuery.value }
    },
    {
      key: "today-sales",
      label: "今日成交额",
    ...
    },
    {
      ...
      analysisPath: isTenantAdmin.value
        ? { path: "/analytics/order", query: workspaceRouteQuery.value }
        : "/analytics/user",
      actionPath: isTenantAdmin.value ? { path: "/order/info", query: workspaceRouteQuery.value } : "/analytics/user"
    },
    {
      key: "today-goods",
      label: "今日商品销量",
      ...
      analysisPath: isTenantAdmin.value ? { path: "/goods/info" } : "/analytics/goods",
    ...
    },
    {
      ...
      analysisPath: { path: "/comment/info" },
...
const todoItems = computed<WorkspaceTodoItem[]>(() => {
  return [
    ...
    },
    {
      ...
      badge: "互动",
      path: { path: "/comment/info", query: { has_pending_discussion: "1" } }
    },
    {
      key: "todo-stock",
      title: "低库存商品",
...
const riskItems = computed<WorkspaceRiskItem[]>(() => {
  const items: WorkspaceRiskItem[] = [
    ...
    },
    {
      ...
      level: "warning",
      levelLabel: "需跟进",
      path: { path: "/comment/info", query: { max_goods_score: "2" } }
    }
  ];
...
function handleOpenCommentDetail(commentId: number) {
  ...
  void navigateTo(router, `/comment/info/detail/${commentId}`);
}
