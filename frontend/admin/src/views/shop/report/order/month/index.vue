...
import MetricCards, { type MetricCardItem } from "@/views/shop/analytics/components/MetricCards.vue";
import PageLayout from "@/views/shop/analytics/components/PageLayout.vue";
import { defOrderReportService } from "@/api/shop/order_report";
import { defTenantStoreService } from "@/api/shop/tenant_store";
import type { OrderMonthReportItem, SummaryOrderMonthReportResponse } from "@/rpc/shop/admin/v1/order_report";
...
async function loadData() {
  ...
  try {
    ...
    const [summaryData, listData] = await Promise.all([
      ...
      defOrderReportService.ListOrderMonthReport(request)
    ]);
    const summary = summaryData;
    const items = listData.order_month_reports ?? [];
    report.summary = {
      ...emptySummary(),
      ...summary,
