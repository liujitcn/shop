<template>
  <div v-if="message.blocks?.length" class="ai-flow-blocks">
    <section v-for="(block, blockIndex) in message.blocks" :key="resolveBlockKey(block, blockIndex)" class="ai-flow-block">
      <div class="ai-flow-block__head">
        <div class="ai-flow-block__title">{{ resolveBlockTitle(block) }}</div>
        <span class="ai-flow-block__type">{{ block.type }}</span>
      </div>
      <p v-if="block.desc" class="ai-flow-block__desc">{{ block.desc }}</p>

      <div v-if="resolveMetricItems(block).length" class="ai-flow-metrics">
        <div v-for="(item, itemIndex) in resolveMetricItems(block)" :key="itemIndex" class="ai-flow-metric">
          <span>{{ item.label }}</span>
          <strong>{{ item.value }}{{ item.unit }}</strong>
        </div>
      </div>

      <div v-for="section in resolveSections(block)" :key="section.key" class="ai-flow-section">
        <div class="ai-flow-section__title">{{ section.title }}</div>
        <template v-if="section.kind === 'list'">
          <div class="ai-flow-list">
            <article v-for="(item, itemIndex) in section.items" :key="itemIndex" class="ai-flow-list__item">
              <div class="ai-flow-list__content">
                <div class="ai-flow-list__title">{{ resolveItemTitle(item, itemIndex) }}</div>
                <div class="ai-flow-list__fields">
        ...
        <template v-else>
          <div class="ai-flow-fields">
            <div v-for="field in section.fields" :key="field.key" class="ai-flow-field">
      ...
      <div v-if="isShipmentFormBlock(block)" class="ai-flow-shipment">
        <div class="ai-flow-shipment__grid">
          <label class="ai-flow-shipment__field">
            <span>物流公司</span>
            <el-input
      ...
      <div v-else-if="resolveActions(block).length" class="ai-flow-actions">
        <el-button
...
import type { AIFlowBlock, ChatMessageItem } from "../types";
...
const emit = defineEmits<{
  ...
  "flow-action": [action: AiAction, label?: string];
}>();
...
/** 生成卡片稳定键。 */
function resolveBlockKey(block: AIFlowBlock, index: number) {
  return [block.type, block.title, index].filter(Boolean).join(":");
}

/** 生成卡片标题。 */
function resolveBlockTitle(block: AIFlowBlock) {
...
/** 返回指标类数据。 */
function resolveMetricItems(block: AIFlowBlock) {
...
/** 将卡片扩展字段拆成可展示分区。 */
function resolveSections(block: AIFlowBlock): DisplaySection[] {
  ...
  Object.entries(block).forEach(([key, value]) => {
    ...
    if (Array.isArray(value) && listFieldKeys.has(key)) {
      const items = value.filter(isRecord) as AIFlowBlock[];
...
/** 返回对象列表项标题。 */
function resolveItemTitle(item: AIFlowBlock, index: number) {
...
function resolveShipmentFormState(block: AIFlowBlock, blockKey: string) {
...
/** 更新发货表单单字段。 */
function updateShipmentFormField(block: AIFlowBlock, blockKey: string, field: keyof ShipmentFormState, value: string | number) {
  resolveShipmentFormState(block, blockKey)[field] = String(value ?? "");
}

/** 判断发货表单当前是否可提交。 */
function isShipmentSubmitDisabled(block: AIFlowBlock, blockKey: string) {
  const action = resolveShipmentAction(block);
  const form = resolveShipmentFormState(block, blockKey);
...
/** 从发货卡片中读取已有物流数据。 */
function resolveShipmentLogistics(block: AIFlowBlock) {
...
/** 生成动作按钮文案。 */
function resolveActionLabel(action: AiAction, source?: Record<string, any>) {
...
function readActionPayload(action: AiAction) {
  try {
...
function isActionEnabled(action?: AiAction) {
...
.ai-flow-metric strong,
.ai-flow-field strong {
  display: block;
  margin-top: 2px;
...
.ai-flow-shipment__grid {
  display: grid;
  grid-template-columns: repeat(3, minmax(0, 1fr));
  gap: 8px;
}
.ai-flow-shipment__field {
  display: flex;
  flex-direction: column;
  gap: 6px;
  min-width: 0;
