        ...
        <ProTable :key="skuTableKey" :data="formData.skuList" :columns="skuColumns" :pagination="false" :tool-button="false">
          <template #skuCode="scope">
            <el-form-item :prop="'skuList[' + scope.$index + '].skuCode'" :rules="rules.skuCode">
              <el-input v-model="scope.row.skuCode" placeholder="请输入规格编号" />
            </el-form-item>
          </template>
...
const state = reactive({
  rules: {
    skuCode: [{ required: true, message: "请输入规格编号", trigger: "blur" }],
    price: [{ required: true, message: "请输入售价", trigger: "blur" }],
    discountPrice: [{ required: true, message: "请输入折后价", trigger: "blur" }],
...
const skuColumns = computed<ColumnProps[]>(() => {
  ...
  return [
    ...
    { prop: "discountPrice", label: "折后价", align: "center", minWidth: 140 },
...
async function submitForm() {
  ...
  // 提交前把动态规格列重新收敛成接口需要的 specItem 数组。
  skuList.forEach((obj: SkuItem) => {
