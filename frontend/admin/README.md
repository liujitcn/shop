...
- 后台业务页默认采用低装饰、稳定的信息管理风格，页面背景以浅灰底为主，同时要兼容暗黑模式、灰色模式、色弱模式。
- 工作台、数据分析、商品详情、商品新增、商品编辑、订单详情，以及订单发货 / 退款相关弹窗，当前已按统一页面风格收敛。
- 工作台 `/dashboard/workspace` 当前按分块接口加载数据，分别对应顶部指标、待处理事项、风险提醒 3 个接口；后续扩展工作台时，优先沿用分块聚合方式，不要回退成单个超大接口。
- 详情页、编辑页、分析页优先使用常规卡片分组，不再新增大面积渐变背景、重装饰 Hero 区和展示型大卡片。
- 后续新增同类页面时，应优先延续统一的卡片圆角、边框、留白和摘要区样式。
- 页面样式优先复用 `src/styles/common.scss` 与 `src/styles/element-dark.scss` 中的业务主题变量，不要在页面内继续写死大段浅色常量，避免暗黑模式、灰色模式、色弱模式下出现对比异常。
- 风格统一设计方案文档见 `docs/admin-page-style-unification-plan.md`。
...
- `/profile` 页面当前拆分为账号信息、安全设置、修改密码三个独立模块；无论从哪个入口进入，页面默认展示“账号信息”，页内标签切换也不再通过路由参数传递。
...
- 商品新增/编辑页当前统一采用三步工作台结构：`商品信息 -> 商品属性 -> 商品库存`，页面层负责步骤编排与摘要展示，各步骤组件分别负责字段录入、属性编辑和规格/SKU 维护。
