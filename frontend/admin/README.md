...
- 聊天区会先本地回显用户消息，再显示助手“思考中”占位态；助手回复会通过 SSE 流式逐段渲染，最终再与服务端正式消息收敛，并按 `reply_source` 区分模型回答、工具回答和降级回复，只在最终助手消息中展示服务端返回的真实 token 用量和工具列表。
- 聊天气泡提供消息级操作：失败的用户消息支持重新发送，成功用户消息支持复制和删除；助手回复支持重新生成、从当前消息创建持久化分支会话、浏览器朗读、复制和删除。删除消息、重新发送、重新生成和创建分支都走后端接口持久化；复制和朗读只在浏览器侧完成。
