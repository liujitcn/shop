...
const emit = defineEmits<{
  ...
  'sku-num-change': [sku: AIFlowBlock, delta: number]
  'sku-submit': [block: AIFlowBlock, sku: AIFlowBlock]
  'create-address-guide': [block: AIFlowBlock]
  'address-submit': [block: AIFlowBlock]
  'address-region-change': [
    block: AIFlowBlock,
    event: Parameters<UniHelper.RegionPickerOnChange>[0],
  ]
  'address-city-change': [block: AIFlowBlock, event: Parameters<UniHelper.UniDataPickerOnChange>[0]]
  'review-submit': [block: AIFlowBlock]
  'review-score-change': [form: AIFlowBlock, key: string, delta: number]
}>()
...
function visibleFlowList(block: AIFlowBlock, blockIndex: number, field: string) {
...
function isAddressFormFieldFilled(block: AIFlowBlock, key: AddressFormStepKey) {
...
function resolveAddressActiveStepKey(block: AIFlowBlock) {
  return (
    props.addressFormSteps.find((item) => !isAddressFormFieldFilled(block, item.key))?.key ?? ''
  )
}

function resolveAddressStepIndex(block: AIFlowBlock) {
...
function resolveAddressStepClass(block: AIFlowBlock, key: AddressFormStepKey) {
...
function resolveAddressGuideHint(block: AIFlowBlock) {
  const activeStepKey = resolveAddressActiveStepKey(block)
  if (!activeStepKey) {
...
function isAddressFormReady(block: AIFlowBlock) {
  return props.addressFormSteps.every((item) => isAddressFormFieldFilled(block, item.key))
}

function forwardFlowAction(action?: AiAction, label?: string) {
...
function isActionEnabled(action?: Partial<AiAction>) {
...
function emitSkuSubmit(block: AIFlowBlock, sku: AIFlowBlock) {
...
function emitSkuNumChange(block: AIFlowBlock, sku: AIFlowBlock, delta: number) {
  if (!isActionEnabled(block.action)) {
    return
  }
...
function emitCreateAddressGuide(block: AIFlowBlock) {
  if (!isActionEnabled(block.action)) {
    return
  }
...
function emitAddressSubmit(block: AIFlowBlock) {
  if (!isActionEnabled(block.action)) {
    return
  }
...
function emitReviewSubmit(block: AIFlowBlock) {
  if (!isActionEnabled(block.action)) {
    return
  }
...
function emitReviewScoreChange(block: AIFlowBlock, key: string, delta: number) {
  if (!isActionEnabled(block.action)) {
    return
  }
...
function resolveSimpleItemImage(item: AIFlowBlock) {
  if (item.picture) {
    return item.picture
  }
