...
const business_license = computed(() => {
  const arr: FileInfo[] = []
  form.value.business_license?.map((url) => {
    const fileInfo = getFileInfo(url)
    arr.push(fileInfo)
  })
  return arr
...
const handlePictureSelect = async (files: any) => {
  ...
  // #ifdef H5 || APP-PLUS
  const validFiles = tempFiles.filter((file: any) => file.size <= imgMaxSize.value)
  ...
  const multiRes = await multiUploadFile('store', validFiles)
  multiRes.map((file) => {
...
const handleBusinessLicenseSelect = async (files: any) => {
  ...
  const multiRes = await multiUploadFile('store', validFiles)
  multiRes.map((file) => {
    form.value.business_license.push(file.url)
  })
  // #endif
  // #ifdef MP-WEIXIN
  const filePaths: string[] = []
...
const handleBusinessLicenseDelete = (file: any) => {
  const tempFilePath = file.tempFilePath
  const index = form.value.business_license.findIndex((item) => tempFilePath.endsWith(item))
  if (index !== -1) {
    form.value.business_license.splice(index, 1) // 同步更新 fileList
  }
}
...
const onRegionChange: UniHelper.RegionPickerOnChange = (ev) => {
  // 省市区(前端展示)
  form.value.address_name = ev.detail.value
  // 省市区(后端参数)
...
<template>
  ...
  <view class="content">
    <uni-forms ref="formRef" :rules="rules" :model="form">
      ...
      <!-- 营业执照（单图） -->
      <uni-forms-item name="business_license" class="form-item">
