...
import { defGoodsInfoService } from '@/api/shop/goods_info.ts'
import { defRecommendService } from '@/api/shop/recommend'
import { defTenantStoreService } from '@/api/shop/tenant_store'
import type { GoodsInfoResponse } from '@/rpc/shop/app/v1/goods_info'
...
import { defShopServiceService } from '@/api/shop/shop_service.ts'
import type { ShopService } from '@/rpc/shop/app/v1/shop_service.ts'
...
const updateActiveSection = (currentScrollTop: number) => {
  ...
  let nextActiveSection: SectionKey = 'goods'

  for (const item of sectionTabs.slice(1)) {
    const sectionOffset = sectionOffsetMap.value[item.key]
    if (sectionOffset > 0 && scrollThreshold >= sectionOffset) {
      nextActiveSection = item.key
    }
  }
...
<template>
  ...
  >
    ...
    <view id="comment-section" class="section-anchor" />
    <CommentPanel
      :goods_id="goodsInfo!.id"
      :goods_picture="formatSrc(goodsInfo!.picture)"
      :sku_code="goodsInfo!.sku_list?.[0]?.sku_code"
    />
    ...
    <view class="detail panel">
      ...
      <view class="content">
        <view class="properties">
          <!-- 属性详情 -->
          <view v-for="item in goodsInfo!.prop_list" :key="item.label" class="item">
