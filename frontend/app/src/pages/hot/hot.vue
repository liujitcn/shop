...
const getShopHotItem = async () => {
  ...
  subTypes.value = res.shop_hot_items || []
  subTypes.value.map((item) => {
    ...
    defShopHotService
      ...
      .then((res) => {
        item.goods_items = res.goods_infos || []
        item.total = res.total || 0
        item.finish = item.total === 0 || item.goods_items.length < 10
        console.log(item)
      })
  })
}
...
const onScrollToLower = async () => {
  ...
  // 分页条件
  if (currSubTypes.goods_items!.length < currSubTypes.total!) {
    // 当前页码累加
    currSubTypes.page_num!++
  } else {
    // 标记已结束
  ...
  const res = await defShopHotService.PageShopHotGoods({
    /** 选项id */
    hot_item_id: currSubTypes.id,
    /** 当前页码 */
    page_num: currSubTypes.page_num!,
    /** 每一页的行数 */
    page_size: currSubTypes.page_size!,
  })
  const list = res.goods_infos || []
  // 数组追加
  currSubTypes.goods_items!.push(...list)
}
</script>
...
<template>
  <view class="viewport">
    ...
    >
      <view v-if="item.goods_items?.length" class="goods">
        <navigator
          v-for="goods in item.goods_items"
          :key="goods.id"
          hover-class="none"
          class="navigator"
      ...
      <XtxEmptyState
        ...
        min-height="520rpx"
      />
      <view v-if="item.goods_items?.length || !item.finish" class="loading-text">
