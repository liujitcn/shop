// Code generated by protoc-gen-ts_proto. DO NOT EDIT.
// versions:
//   protoc-gen-ts_proto  v2.11.8
//   protoc               unknown
// source: shop/app/v1/pay.proto

/* eslint-disable */
import type { Empty } from "../../../google/protobuf/empty";
import type { Timestamp } from "../../../google/protobuf/timestamp";

/** 支付通知请求参数 */
export interface PayNotifyRequest {
}

/** 小程序支付请求参数 */
export interface JsapiPayRequest {
  /** 交易单ID */
  trade_id: number;
}

/** H5支付请求参数 */
export interface H5PayRequest {
  /** 交易单ID */
  trade_id: number;
}

/** 小程序支付响应 */
export interface JsapiPayResponse {
  /** 填写下单时传入的appid，且必需与当前实际调起支付的公众号appid一致，否则无法调起支付。 */
  app_id: string;
  /** Unix 时间戳，是从1970年1月1日（UTC/GMT的午夜）开始所经过的秒数。注意：常见时间戳为秒级或毫秒级，该处必需传秒级时间戳。 */
  time_stamp: string;
  /** 随机字符串，不长于32位。该值建议使用随机数算法生成。 */
  nonce_str: string;
  /** 订单详情扩展字符串，JSAPI下单接口返回的prepay_id参数值，提交格式如：prepay_id=***。 */
  package: string;
  /** 签名，使用字段appId、timeStamp、nonceStr、package计算得出的签名值 注意：取值RSA格式。详细参考JSAPI调起支付签名https://pay.weixin.qq.com/doc/v3/merchant/4012365339 */
  pay_sign: string;
}

/** H5 支付响应 */
export interface H5PayResponse {
  /** 支付url */
  h5_url: string;
}

/** 支付结果 */
export interface PaymentResource {
  /** 【微信支付订单号】微信支付侧订单的唯一标识 */
  transaction_id: string;
  /** 金额信息 */
  amount:
    | PaymentResource_Amount
    | undefined;
  /** 【商户号】商户下单时传入的商户号 */
  mchid: string;
  /** 交易状态 */
  trade_state: PaymentResource_TradeState;
  /** 【银行类型】用户支付方式说明 */
  bank_type: string;
  /** 【支付完成时间】用户完成支付的时间 */
  success_time:
    | Timestamp
    | undefined;
  /** 支付者信息 */
  payer:
    | PaymentResource_Payer
    | undefined;
  /** 【商户订单号】商户系统内部订单号 */
  out_trade_no: string;
  /** 【公众账号ID】下单时传入的公众账号ID */
  appid: string;
  /** 【交易状态描述】状态详细说明 */
  trade_state_desc: string;
  /** 交易类型 */
  trade_type: PaymentResource_TradeType;
  /** 【商户数据包】自定义数据包（<=128字符） */
  attach: string;
  /** 场景信息 */
  scene_info: PaymentResource_SceneInfo | undefined;
}

/** 【交易状态】交易状态枚举 */
export enum PaymentResource_TradeState {
  TRADE_STATE_UNSPECIFIED = 0,
  /** SUCCESS - 支付成功 */
  SUCCESS = 1,
  /** REFUND - 转入退款 */
  REFUND = 2,
  /** NOTPAY - 未支付 */
  NOTPAY = 3,
  /** CLOSED - 已关闭 */
  CLOSED = 4,
  /** REVOKED - 已撤销（仅付款码支付会返回） */
  REVOKED = 5,
  /** USERPAYING - 用户支付中（仅付款码支付会返回） */
  USERPAYING = 6,
  /** PAYERROR - 支付失败（仅付款码支付会返回） */
  PAYERROR = 7,
}

/** 交易类型枚举 */
export enum PaymentResource_TradeType {
  TRADE_TYPE_UNSPECIFIED = 0,
  /** JSAPI - 公众号/小程序支付 */
  JSAPI = 1,
  /** NATIVE - Native支付 */
  NATIVE = 2,
  /** APP - APP支付 */
  APP = 3,
  /** MICROPAY - 付款码支付 */
  MICROPAY = 4,
  /** MWEB - H5支付 */
  MWEB = 5,
  /** FACEPAY - 刷脸支付 */
  FACEPAY = 6,
}

/** 支付金额信息 */
export interface PaymentResource_Amount {
  /** 【用户支付金额】用户支付金额，整型，单位为分。（指使用优惠券的情况下，这里等于总金额-优惠券金额） */
  payer_total: number;
  /** 【总金额】订单总金额，单位为分，整型 */
  total: number;
  /** 【货币类型】固定返回：CNY，代表人民币 */
  currency: string;
  /** 【用户支付币种】 固定返回：CNY，代表人民币。 */
  payer_currency: string;
}

/** 支付者信息 */
export interface PaymentResource_Payer {
  /** 【用户标识】用户在商户appid下的唯一标识 */
  openid: string;
}

/** 支付场景信息 */
export interface PaymentResource_SceneInfo {
  /** 【商户端设备号】门店号/收银设备ID */
  device_id: string;
}

/** 退款结果 */
export interface RefundResource {
  /** 【商户号】商户下单时传入的商户号 */
  mchid: string;
  /** 【微信支付订单号】微信支付侧订单的唯一标识 */
  transaction_id: string;
  /** 【商户订单号】商户系统内部订单号 */
  out_trade_no: string;
  /** 【微信支付退款单号】微信侧生成的唯一退款标识 */
  refund_id: string;
  /** 【商户退款单号】商户系统内部退款单号 */
  out_refund_no: string;
  /** 退款状态 */
  refund_status: RefundResource_RefundStatus;
  /** 【退款成功时间】 */
  success_time:
    | Timestamp
    | undefined;
  /** 【退款入账账户】 */
  user_received_account: string;
  /** 金额信息 */
  amount: RefundResource_Amount | undefined;
}

/** 【退款状态】 */
export enum RefundResource_RefundStatus {
  REFUND_STATUS_UNSPECIFIED = 0,
  /** SUCCESS - 退款成功 */
  SUCCESS = 1,
  /** CLOSED - 退款关闭 */
  CLOSED = 2,
  /** PROCESSING - 退款处理中 */
  PROCESSING = 3,
  /** ABNORMAL - 退款异常 */
  ABNORMAL = 4,
}

/** 退款金额信息 */
export interface RefundResource_Amount {
  /** 【原订单金额】单位：分 */
  total: number;
  /** 【退款金额】单位：分 */
  refund: number;
  /** 【用户实际支付金额】单位：分 */
  payer_total: number;
  /** 【用户退款金额】单位：分 */
  payer_refund: number;
}

/** App支付服务 */
export interface PayService {
  /** 小程序支付 */
  JsapiPay(request: JsapiPayRequest): Promise<JsapiPayResponse>;
  /** H5支付 */
  H5Pay(request: H5PayRequest): Promise<H5PayResponse>;
  /** 支付通知 */
  PayNotify(request: PayNotifyRequest): Promise<Empty>;
}
