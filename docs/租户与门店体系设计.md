...
| 订单边界 | 一张 `order_trade` 聚合一次提交，按门店创建一张或多张 `order_info` 子订单 |
| 统计粒度 | 商品日统计按租户和商品汇总，订单日统计保留租户与门店维度 |
...
erDiagram
  ...
  ORDER_TRADE ||--|{ ORDER_INFO : aggregates
...
| 交易与门店子订单 | `order_trade` 聚合整笔支付和取消，`order_info` 一张只保存一个 `tenant_store_id` 并独立履约。 |
| 租户与门店统计 | 订单日统计保存 `tenant_id + tenant_store_id`，默认租户可汇总全部数据，普通租户只读取本租户结果。 |
...
| `v0` | 租户编码 | `1000` |
| `v1` | 角色编码 | `tenant` |
| `v2` | Kratos operation | `/admin.v1.GoodsInfoService/PageGoodsInfo` |
| `v3` | HTTP Method | `GET` |
| `v4` | 项目或资源域占位 | `*` |
| `v5` | 保留字段 | 空字符串 |
...
```text
p, 1000, tenant, /admin.v1.GoodsInfoService/PageGoodsInfo, GET, *
```
...
当前用户、角色、部门和门店菜单仍关联了 `BaseTenantService/OptionBaseTenant`，因此普通租户可以读取全部启用租户的下拉选项，但不能据此获得租户 CRUD 权限。该跨租户信息暴露不是目标能力，后续应从普通租户策略移除或由接口只返回当前租户。
...
| 订单 | `order_info` | 是 | 是 | 从订单商品所属门店确定，一笔交易可包含多张一单一门店的子订单。 |
| 评价 | `comment_info` | 是 | 是 | 从订单和商品快照继承归属。 |
| 商品统计 | `goods_stat_day` | 是 | 否 | 按租户和商品聚合。 |
| 订单统计 | `order_stat_day` | 是 | 是 | 按租户、门店、支付方式和渠道聚合。 |
...
| `order_trade` | `order_address`、`order_cancel`、`order_payment` | 通过 `trade_id` 获取整笔交易上下文。 |
| `order_info` | `order_goods`、`order_logistics` | 通过 `order_id` 获取租户和门店。 |
| `order_trade + order_info` | `order_refund` | 同时保存 `trade_id + order_id`，以门店子订单退款并向上汇总交易退款状态。 |
| `comment_info` | `comment_discussion`、`comment_reaction`、`comment_review`、`comment_summary`、`comment_tag` | 通过评价、订单或商品关联获取租户和门店。 |

支付和取消是交易事实，物流和退款是门店子订单事实。这些记录不重复保存租户字段；需要租户口径时，必须通过 `trade_id` 和 `order_id` 回查 `order_info`。
...
| `order_stat_day` | `tenant_id + tenant_store_id + stat_date + pay_type + pay_channel` 唯一。 |
...
购物车继续按用户维护，`user_cart` 不保存租户和门店字段。同一用户可以选中多个门店的商品并在一次交易中结算：

1. 购物车、确认单、立即购买和再次购买都由后端根据 `tenant_store_id` 返回门店分组，前端不自行分组。
2. 后端校验每个商品已绑定可用门店，创建请求按门店提交配送时间和备注。
3. 一次创建使用同一事务写入一张 `order_trade`，再按门店写入一张或多张 `order_info`。
4. 每张子订单的 `tenant_id` 和 `tenant_store_id` 来自当前分组商品，商品快照和物流关联 `order_id`。
5. 地址、支付和取消关联 `trade_id`；退款同时关联 `trade_id + order_id`。
6. 创建成功后返回 `trade_id`、`trade_no` 和全部 `order_ids`，并删除本次已选中的购物车项。
7. 单门店和多门店使用同一创建、支付和取消流程。

### 订单、评价与统计

- 后台订单列表、详情、分析和工作台指标通过 `order_info.tenant_id` 隔离；默认租户使用租户/门店树筛选全部数据，普通租户使用本租户门店下拉筛选。
...
- `order_stat_day` 按 `tenant_id + tenant_store_id + stat_date + pay_type + pay_channel` 聚合订单指标。
...
| `app.v1.OrderInfoService` | 执行确认单、创建、交易聚合查询与门店子订单履约，约束见“购物车与多门店下单”。 |
...
- 商品、订单、评价列表在平台视角使用“租户 -> 门店”两级树筛选，普通租户使用本租户门店下拉筛选。
- 商品编辑必须选择审核通过的门店。
...
- 购物车返回 `user_cart_stores`，确认单、立即购买和再次购买返回 `order_goods_stores`，所有分组由后端完成。
- 未支付订单列表展示交易下全部门店商品，支付后按门店子订单展示门店信息和履约操作。
...
| 租户下拉信息暴露 | 普通租户菜单关联了 `OptionBaseTenant` 时，可以调用接口读取启用租户选项。 | 从普通租户策略移除无用的跨租户选项权限，或让接口在普通租户上下文只返回当前租户。 |
...
| 跨门店购物车 | 一次可选择多个门店并统一结算。 | 后端固定返回门店分组，并在创建时校验每个门店的配送选项和商品归属。 |
| 租户编码并发生成 | 并发创建可能命中唯一索引冲突。 | 保留唯一索引；必要时增加重试或编码分配锁。 |
| 默认租户全局汇总 | 平台查询不自动追加租户条件。 | 统计只维护一套口径，平台使用租户/门店树缩小范围，普通租户由上下文强制隔离。 |
...
- 跨门店商品在确认单中按门店分组，创建后只产生一张交易单和多张门店子订单。
- 单门店与多门店交易都使用 `trade_id` 支付和取消，使用 `order_id` 履约、评价和门店退款。
- 订单和评价同时保存正确的 `tenant_id`、`tenant_store_id`，相关明细可通过主表推导归属。
- 租户存在门店、商品、订单或评价时不能删除。
- 租户级商品统计和租户/门店级订单统计互不串租户，默认租户可以汇总全部结果。
...
- 优惠分摊：若后续增加交易级优惠，需明确门店子订单金额分摊和退款回收规则。
- 门店级商品统计：若需要从租户商品维度下沉到门店，再扩展 `goods_stat_day` 与历史重算能力。
