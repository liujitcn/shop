...
默认初始化会写入 `default` 租户。角色编号固定为 `super(1)`、`tenant(2)`、`admin(3)`、`user(4)`、`guest(5)`，其中 `tenant` 是租户管理员内置角色，菜单只包含工作台、订单分析、个人资料、组织权限、商品、评价和订单等租户经营能力，不包含租户管理、菜单/API/任务/配置、商城服务、推荐、支付账单和报表等平台能力。
