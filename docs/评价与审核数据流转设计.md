...
5. LLM 异常或未配置：主表保持 `status = 1`，写入异常审核记录。`CommentAuditRetry` 定时任务会补偿重新投递仍处于待审核的异常记录；超过最大异常次数后保留给人工处理。
...
4. LLM 异常或未配置：讨论保持待审核，只写异常审核记录。`CommentAuditRetry` 定时任务会补偿重新投递仍处于待审核的异常记录；超过最大异常次数后保留给人工处理。

### 自动审核补偿

`CommentAuditRetry` 每 5 分钟默认执行一次，只重新投递 `COMMENT_AUDIT` 队列，不直接调用 LLM。补偿范围限定为仍处于 `PENDING_REVIEW_CS` 的评价和讨论：
...
- 单次默认分别最多投递评价 50 条、讨论 50 条，可通过任务参数 `batchSize`、`retryDelayMinutes`、`maxRetry` 调整。
