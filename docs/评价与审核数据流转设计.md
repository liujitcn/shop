...
本文档说明商品评价、讨论、标签、评价摘要、审核记录和互动计数的最新数据流转方案。当前方案以“先保存待审核、再异步 LLM 审核、异常转人工”为主线，减少审核表冗余字段，并把前台高频展示计数落到主表缓存。

## 能力边界

| 模块 | 责任 |
| --- | --- |
| 商城端 | 展示已审核评价、评价标签、评价摘要；提交评价、讨论；点赞、点踩。 |
| 后端 App 服务 | 保存评价 / 讨论为待审核；投递 LLM 审核队列；提供前台查询和互动接口。 |
| LLM / AI 能力 | 审核评价 / 讨论，返回通过、不通过或异常；通过时返回标签名称；刷新商品评价摘要。 |
| 管理后台 | 查看评价详情、讨论列表、审核记录；人工设置评价 / 讨论通过或不通过。 |
| 数据库 | 保存主表状态、统一审核记录、标签、评价摘要和互动明细，并维护展示计数缓存。 |
...
3. LLM 通过：
   ...
   - 写入 `comment_review`；
   - 投递 `COMMENT_SUMMARY_REFRESH` 刷新商品评价摘要。
4. LLM 不通过：更新 `comment_info.status = 3`，写入 `comment_review.reason`。
5. LLM 异常或未配置：主表保持 `status = 1`，写入异常审核记录。`CommentAuditRetry` 定时任务会同步补偿审核仍处于待审核的异常记录；超过最大异常次数后保留给人工处理。
...
2. LLM 通过：更新讨论为 `status = 2`，父评价 `pending_discussion_count - 1`、`discussion_count + 1`，写审核记录，并刷新商品评价摘要。
3. LLM 不通过：更新讨论为 `status = 3`，父评价 `pending_discussion_count - 1`，写审核记录。
4. LLM 异常或未配置：讨论保持待审核，只写异常审核记录。`CommentAuditRetry` 定时任务会同步补偿审核仍处于待审核的异常记录；超过最大异常次数后保留给人工处理。

### 自动审核补偿

`CommentAuditRetry` 每 5 分钟默认执行一次，不再重新投递 `COMMENT_AUDIT` 队列，而是直接同步执行 LLM 审核。补偿范围限定为仍处于 `PENDING_REVIEW_CS` 的评价和讨论：

- 创建时间早于当前时间 3 分钟，避免刚提交的数据和实时队列并发重复处理。
- 最近一次 AI 审核不存在，或最近一次 AI 审核为 `COMMENT_REVIEW_STATUS_EXCEPTION` 且异常时间已超过 3 分钟。
- 同一目标 AI 异常次数少于 3 次，避免配置错误、图片永久读取失败等场景无限重试。
- 单次默认分别最多同步审核评价 50 条、讨论 50 条，可通过任务参数 `batchSize`、`retryDelayMinutes`、`maxRetry` 调整。

## 人工审核流程

管理后台只允许人工设置“通过”或“不通过”，不主动写“待审核”。

- 评价人工通过：更新 `comment_info.status = 2`，写 `comment_review(type=2,status=1)`，刷新商品评价摘要。
- 评价人工不通过：更新 `comment_info.status = 3`，写 `comment_review(type=2,status=2)`；如果原状态为通过，则扣减已命中标签提及次数并清空 `tag_id`，再刷新商品评价摘要。
- 讨论人工通过 / 不通过：更新 `comment_discussion.status`，同步调整父评价讨论计数，写审核记录；通过时刷新商品评价摘要。
...
- `comment_summary` 按商品和场景保存摘要：
  - `OVERVIEW`：商品详情评价概览卡片；
  - `LIST`：评价列表页摘要。
- 评价摘要刷新失败不影响评价审核主流程，前台可降级展示历史摘要或普通评价列表。
...
| `comment_summary` | `like_count` / `dislike_count` | 评价摘要点赞 / 点踩展示数。 |
