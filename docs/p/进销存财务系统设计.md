...
- 当前销售端已经支持货到付款，订单创建、付款页和履约状态均需纳入应收、收款及库存流程设计。
...
| 业务域 | 职责 | 不负责 |
| --- | --- | --- |
| `sales` | 销售下单、支付、履约、退款、售后入口 | 采购、库存成本、供应商往来、商户打款 |
...
当前销售端已经支持货到付款，因此货到付款应收、签收收款、拒收/取消释放预占和退货流程属于正式实施范围。企业客户赊销仍为预留能力，待企业客户档案、账期、授信和逾期控制完整后启用。
...
- `backend/Makefile` 中管理端和通用端的 Proto 排除规则，避免通用端生成后台财务契约。
